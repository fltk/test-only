...
void save_cb(Fl_Widget *, void *v) {
  ...
  if (v || !c || !*c) {
/// \todo CRITICAL    fl_file_chooser_ok_label("Save");
...
void open_cb(Fl_Widget *, void *v) {
  ...
  const char *c;
  const char *oldfilename;
/// \todo CRITICAL  fl_file_chooser_ok_label("Open");
...
void print_cb(Fl_Return_Button *, void *) {
  ...
  if (print_choice->value()) {
    ...
    // Print to file...
/// \todo CRITICAL    fl_file_chooser_ok_label("Print");
    const char *outname = fl_file_chooser("Print To", "PostScript (*.ps)", NULL, 1);
// \todo CRITICAL    fl_file_chooser_ok_label(NULL);
