...
// modify it under the terms of the GNU Library General Public Licens
...
#define sorceress_width 7
#define sorceress_height 7
...
void make_image() 
  ...
  for (int y = 0; y < height; y++) 
    ...
    for (int x = 0; x < width; x++) 
      ...
      *p++ = uchar(255*((1-X)*(1-Y))); // red in upper-lef
      *p++ = uchar(255*((1-X)*Y));	// green in lower-lef
      *p++ = uchar(255*(X*Y));	// blue in lower-righ
      X -= 0.5
      Y -= 0.5
      int alpha = (int)(350 * sqrt(X * X + Y * Y))
      if (alpha < 255) *p++ = uchar(alpha);	// alpha transparenc
      else *p++ = 255
      Y += 0.5
...
void print(Fl_Widget *, void *w) 
    ...
    c->set_current()
    delete p
    fclose(f); //we need close file her
}
...
void print2(Fl_Widget *, void *w) 
  ...
  if(!p) return
  p->page(Fl_Printer::A4)
  // fitting inside margins 1 inch wid
