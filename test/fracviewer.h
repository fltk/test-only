 ...
 * The two view movement modes are POLAR and FLYING.  Both move the eye, NO
 * THE OBJECT.  You can never be upside down or twisted (roll) in either mode
 
 * A nice addition would be an examiner type trackball mode where you ar
 * moving the object and so could see it from any angle.  Also less restricte
 * flying and polar modes (fly upside down, do rolls, etc.)
 
 * Controls for Polar are just left and middle buttons -- for flying it'
 * those plus 0-9 number keys and +/- for speed adjustment
 
 * See agv_example.c and agviewer.c for more info.  Probably want to mak
 * a copy of these and then edit for each program.  This isn't meant to b
 ...
 *
  ...
  * which you want to run the viewer. Right after creating it is fine.  I
  * will remember that window for possible later use (see below) an
  * registers mouse, motion, and keyboard handlers for that window (see below)
  
  * allowidle is 1 or 0 depnding on whether you will let AGV instal
  * and uninstall an idle function.  0 means you will not let it (becaus
  * you will be having your own idle function). In this case it is you
  * responsibility to put a statement like
  
  *     if (agvMoving
  *       agvMove()
  
  * at the end of your idle function, to let AGV update the viewpoint if i
  * is moving.
  ...
  * agvSetIdleAllow changes this value so you can let AGV install its idl
...
void agvSwitchMoveMode(int move)
  ...
  * current position.  So call it in your display on the projection matri
  *
void agvViewTransform(void)

 /
  * agvMoving will be set by AGV according to whether it needs you to cal
...
void agvMove(void)
  ...
  * These are all registered with agvInit(), but you could registe
...
void agvHandleKeys(unsigned char key, int x, int y)
  ...
  * which is nice for aligning things and debugging.  Pass it an availabl
  * displaylist number
  *
