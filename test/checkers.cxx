...
// Define FLTK to get the fltk interfac
// Define VT100 to get the VT100 interfac
// Define both to get a program that takes a -t switc
...
int maxevaluate=2500;		// max number of moves to examine on a tur
int maxnodes = 2500;		// maximum number of nodes in search tre
int maxply = 20;		// maximum depth to look ahea
char forcejumps = 1;		// is forced jumps rule in effect

// scoring parameters: (all divided by 5 from original code
// some signs seem to be backwards, marked them with (-) in commen
const int spiece = 800;		// value of a piec
const int sking = 1200;		// value of a kin
const int sadvan = 160;		// value of mypieces/theirpieces-
// const int smobil = ?		// moves *enemy* can make w/o being jumpe
const int sallpin = 80;		// mobil == 
const int sdeny = 10;		// moves enemy can make that will be jumpe
const int spin = 32;		// enemy pieces that have no move except jumpe
const int sthreat = -10;	// enemy pieces we can jump if not moved (-
const int sgrad = 1;		// score of piece position
const int sback = 10;		// back row occupied so enemy can't make kin
const int smoc2 = 200;		// more mobility, more cente
const int smoc3 = -8;		// less mobility, less cente
const int smoc4 = -80;		// more mobility, less cente
const int smode2 = -14;		// less mobility, less denie
const int smode3 = -40;		// more mobility, more denied (-
const int sdemmo = -20;		// more denied, more moves (-
const int scent = 10;		// pieces in cente
const int skcent = 100;		// kings in cente

const int depthpenalty=4;	// gues
const int noise=2;		// values less or eq to this apart are e

// const int sattackking = 4;	// not use
// const int sattackpiece = 3

struct node 
  node *father
  node *son;		// best so
  node *brother;	// next brothe
  short int value;	// value of this board position to player making mov
  unsigned char from,to; // the move to reach this boar
  long int jump;	// bit map of locations jumpe
  unsigned char mobil
  unsigned char deny
  unsigned char pin
  unsigned char threat
  short int gradient
  unsigned who:1;	// 0 = black's move, 1 = white's mov
  unsigned king:1;	// 1 = move causes piece to be kinge
  unsigned back:1
  unsigned moc2:1
  unsigned moc3:1
  unsigned moc4:1
...
int nodes;		// count of node

/*	Board positions:	Border positions

	      WHITE		  00  01  02  03  0
	  05  06  07  08	04  XX  XX  XX  X
	09  10  11  12		  XX  XX  XX  XX  1
	  14  15  16  17	13  XX  XX  XX  X
	18  19  20  21		  XX  XX  XX  XX  2
	  23  24  25  26	22  XX  XX  XX  X
	27  28  29  30		  XX  XX  XX  XX  3
	  32  33  34  36	31  XX  XX  XX  X
	36  37  38  39		  XX  XX  XX  XX  4
	      BLACK		40  41  42  43  4
...
const int offset[9][4] = {	// legal move direction
  {0,0,0,0}
  {-5,-4,0,0}
  {4,5,0,0}
  {0,0,0,0}
...
piece b[45];		// current board position being considere

int evaluated;		// number of moves evaluated this tur

char centralsquares[45]
char is_protected[45]

piece flipboard[45];	// swapped if enemy is blac
piece *tb;		// pointer to real or swapped boar
#define FRIEND BLAC
#define FRIENDKING BLACKKIN
#define ENEMY WHIT
#define ENEMYKING WHITEKIN

char check(int target,int direction) 
  // see if enemy at target can be jumped from direction by our piec
  ...
  int safe 
    (tb[src-4]&FRIEND && tb[src-8]&ENEM
     ||tb[src-5]&FRIEND && tb[src-10]&ENEM
     ||tb[dst-4]&ENEMY && !tb[dst+4
     ||tb[dst-5]&ENEMY && !tb[dst+5
     ||tb[src+4]&FRIEND && tb[src+8]==ENEMYKIN
     ||tb[src+5]&FRIEND && tb[src+10]==ENEMYKIN
...
void expandnode(node *f) 
  if (f->son || f->value > 28000) return;	// already don
...
node *calcmove(node *root) {	// return best move after roo
  expandnode(root)
  if (!root->son) return(0);	// no move due to los
...
piece jumpboards[24][45];	// saved boards for undoing jump
int nextjump

char user;	// 0 = black, 1 = whit
char playing
char autoplay
...
node* undomove() 
  node *n = root
  if (n == undoroot) return 0; // no more undo possibl
  ...
  killnode(n)
  root->son = 0
  root->value = 0;	// prevent it from thinking game is ove
  playing = 1
...
//---------------------------------------------------------------
// old 4-level NeXT images have been seperated into bitmaps so the
// can be drawn with arbitrary colors and real transparency.  This i
// rather tedious and perhaps fltk should provide a direct suppor
// to do this
...
static int erase_this;  // real location of dragging piece, don't draw i
static int dragging;	// piece being dragge
static int dragx;	// where it i
static int dragy
static int showlegal;	// show legal move
...
void Board::drag_piece(int j, int dx, int dy) 
  dy = (dy&-2) | dx&1; // make halftone shadows line u
  if (j != erase_this) drop_piece(erase_this); // should not happe
  if (!erase_this) { // pick up old piec
...
// drop currently dragged piece on square 
...
int busy; // causes pop-up abort men
...
int Board::handle(int e) 
  ...
  case FL_RELEASE
    ...
    if (!n) {drop_piece(erase_this); return 1;} // none foun
    drop_piece(n->to)
    domove(n)
    if (showlegal) {showlegal = 0; redraw();
    if (n->jump) redraw()
