...
/
// This demo shows how to do many of the window manipulations tha
// are popular on SGI programs, even though X does not really lik
// them.  You can toggle the border on/off, change the visual t
// switch between single/double buffer, and make the window tak
// over the screen
/
// Normally the program makes a single window with a child GL window
// This simulates a program where the 3D display is surrounded b
// control knobs.  Running the program with an argument wil
// make it make a seperate GL window from the controls window.  Thi
// simulates a (older?) style program where the graphics display i
// a different window than the controls
/
// This program reports how many times it redraws the window t
// stdout, so you can see how much time it is wasting.  It appear
// to be impossible to prevent X from sending redundant resiz
// events, so there are extra redraws.  But the way I have th
...
/
// Turning off full screen when the border is on causes an unnecessar
// resize and redraw when the program turns the border on
/
// If it is a seperate window, turning double buffering on and of
// will cause the window to raise, deiconize, and possibly move.  Yo
// can avoid this by making the Fl_Gl_Window a child of a norma
// window
/
...
int main(int argc, char **argv) 
  ...
  Fl_Window *w
  if (twowindow) {	// make it's own windo
    sw.resizable(&sw)
    w = &sw
    window.set_modal();	// makes controls stay on top when fullscreen pushe
    argc--
    sw.show()
  } else {		// otherwise make a subwindo
    window.add(sw)
    window.resizable(&sw)
    w = &window
