...
/
// This is a GLUT demo program, with modifications t
// demonstrate how to add FLTK controls to a GLUT program.   The GLU
...
#els
/
 * To compile: cc -o fractals fractals.c -lGL -lGLU -lX11 -lglut -lXmu -l
 ...
 * Fog would make the island look much better, but I couldn't get it to wor
 * correctly.  Would line up on -z axis not from eye
 
 * Philip Winston - 3/4/9
 * pwinston@hmc.ed
...
#if 0	// removed for FL, it is in fracviewer.
  /* normalizes v *
...
GLfloat DispFactor[MAXLEVEL];  /* Array of what to multiply random numbe
				  by for a given level to get midpoin
				  displacement  *
GLfloat DispBias[MAXLEVEL];  /* Array of what to add to random numbe
				before multiplying it by DispFactor *

#define NUMRANDS 19
float RandTable[NUMRANDS];  /* hash table of random numbers so we ca
...
void Midpoint(GLfloat mid[3], GLfloat v1[3], GLfloat v2[3]
   ...
   * allowing an edge to be pegged.  This function requires the abov
...
void FMR(GLfloat v1[3], GLfloat v2[3], GLfloat v3[3], int level
 ...
 /
  * sets up lookup tables and calls recursive mountain functio
  *
void FractalMountain(GLfloat v1[3], GLfloat v2[3], GLfloat v3[3]
 ...
 /
  * draw a mountain and build the display lis
  *
void CreateMountain(void
  ...
  /
   * new random numbers to make a different moutai
   *
...
void FractalTree(int level
  ...
  /
   * Create display lists for a leaf, a set of leaves, and a ste
   *
void CreateTreeLists(void
 ...
 /
  * draw and build display list for tre
  *
...
void display(void
{
  ...
  /
  // Use glFinish() instead of glFlush() to avoid getting many frame
...
//  glutInit(&argc, argv); // this line removed for FLT
  ...
  window.show(argc,argv); // glut will die unless parent window visibl
  window.begin(); // this will cause Glut window to be a chil
  glutInitWindowSize(512, 512)
  glutInitWindowPosition(10,90); // place it inside parent windo
  ...
  glutMainLoop(); // you could use Fl::run() instea
