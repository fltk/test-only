...
/
// This is a test of the minimal update code.  The right slider has 
// label that extends outside it's border, and the minimal updat
// assummes this does not happen.  Thus there is *supposed* to b
// display errors when you move the right-most or any other slider
// If you *don't* see these errors, then the minimal update i
// broken!!
/
// I cannot emphasize how important it is to test this and make sur
// any changes have not broken the minimal update.  These sort of bug
// are extremely hard to fix and must be detected right away
/
// The reason it is important to fix this is that otherwise you wil
// swiftly end up with a toolkit that thinks it has to draw the windo
// 20 times each time the display changes.  I don't care how fast th
// machine is, this is an insane waste of resources, and should b
// stopped
/
