...
/
// Modified to have 2 cubes to test multiple OpenGL context
/
...
int main(int argc, char **argv) 
  ...
  // In this case the same context is used by multiple windows (I'm no
  // sure if this is allowed on Win32, can somebody check?)
  // This fixes a bug on the XFree86 3.0 OpenGL where only one contex
  // per program seems to work, but there are probably better uses fo
  // this
  cube->make_current(); // causes context to be create
  cube2->context(cube->context()); // share the context
#endi
  for (;;) 
    if (form->visible() && speed->value()
      {if (!Fl::check()) break;}	// returns immediatel
    els
      {if (!Fl::wait()) break;}	// waits until something happen
