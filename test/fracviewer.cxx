 ...
 * AGV: a glut viewer. Routines for viewing a 3d scene w/ glu
 
 * See agv_example.c and agviewer.h comments within for more info
 
 * I welcome any feedback or improved versions
 
 * Philip Winston - 4/11/9
 * pwinston@hmc.ed
 * http://www.cs.hmc.edu/people/pwinsto
 *

#include <config.h

#if HAVE_GL && HAVE_GL_GLU_
#  include <FL/glut.H
#  ifdef __APPLE_
#    include <OpenGL/glu.h
#  els
#    include <GL/glu.h> // added for FLT
#  endi
...
#define INIT_POLAR_AZ  0.
#define INIT_POLAR_EL 30.
#define INIT_DIST      4.
#define INIT_AZ_SPIN   0.
#define INIT_EL_SPIN   0.

  /* Initial flying movement settings *
#define INIT_EX        0.
#define INIT_EY       -2.
#define INIT_EZ       -2.
#define INIT_MOVE     0.0
...
int AllowIdle, RedisplayWindow;
   /* If AllowIdle is 1 it means AGV will install its own idle whic
    * will update the viewpoint as needed and send glutPostRedisplay() to th
    * window RedisplayWindow which was set in agvInit().  AllowIdle of 
    * means AGV won't install an idle funciton, and something lik
    * "if (agvMoving) agvMove()" should exist at the end of the runnin
    * idle function
    *
...
/***************************************************************
  ...
  /
   * these are functions meant for internal use onl
   * the other prototypes are in agviewer.
   *
...
/***************************************************************
  ...
  /
   * viewing transformation modified from page 90 of red boo
   *
void PolarLookFrom(GLfloat dist, GLfloat elevation, GLfloat azimuth
  ...
  /
   * I took the idea of tracking eye position in absolut
   * coords and direction looking in Polar form from deni
   *
void FlyLookFrom(GLfloat x, GLfloat y, GLfloat z, GLfloat az, GLfloat el
  ...
  /
   * Call viewing transformation based on movement mod
   *
void agvViewTransform(void
{
   ...
   * but maybe it wouldn't be too hard to adapt things to let you g
   * upside dow
   *
int ConstrainEl(void
 ...
 /
  * Idle Function - moves eyepositio
  *
void agvMove(void
  ...
  /
   * Don't install agvMove as idle unless we will be updating the vie
   * and we've been given a RedisplayWindo
   *
void MoveOn(int v
  ...
  /
   * set new redisplay window.  If <= 0 it means we are not to instal
   * an idle function and will rely on whoever does install one to
   * put statement like "if (agvMoving) agvMove();" at end of i
   *
void agvSetAllowIdle(int allowidle
  ...
  /
   * when moving to flying we stay in the same spot, moving to polar w
   * reset since we have to be looking at the origin (though a pivot fro
...
void agvHandleButton(int button, int state, int x, int y
 ...
 /
  * change EyeEl and EyeAz and position when mouse is moved w/ button dow
  *
...
/***************************************************************
  ...
  /
   * set EyeMove (current speed) for FLYING mod
   *
void SetMove(float newmove
  ...
  /
   * 0->9 set speed, +/- adjust current speed  -- in FLYING mod
   *
