...
void CodeEditor::style_update(int pos, int nInserted, int nDeleted,
  ...
  if (nInserted > 0) {
    ...
    editor->styleBuffer()->replace(pos, pos + nDeleted, style);
    delete[] style;
  } else {
    // Just delete characters in the style buffer...
    editor->styleBuffer()->remove(pos, pos + nDeleted);
  }

  // Select the area that was just updated to avoid unnecessary
  // callbacks...
  editor->styleBuffer()->select(pos, pos + nInserted - nDeleted);
  ...
  text  = editor->buffer()->text_range(start, end);
  style = editor->styleBuffer()->text_range(start, end);
  if (start==end)
    last = 0;
  else
  ...
  editor->styleBuffer()->replace(start, end, style);
...
CodeEditor::CodeEditor(int X, int Y, int W, int H, const char *L) :
  ...
  highlight_data(new Fl_Text_Buffer(buffer()->length()), styletable,
...
CodeEditor::~CodeEditor() {
  Fl_Text_Buffer *buf = styleBuffer();
  styleBuffer(0);
  delete buf;
