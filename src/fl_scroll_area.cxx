...
// Drawing function to move the contents of a rectangle.  This is passe
// a "callback" which is called to draw rectangular areas that are move
...
void fl_scroll(int X, int Y, int W, int H, int dx, int dy
  ...
  if (dx <= -W || dx >= W || dy <= -H || dy >= H) 
    // no intersection of old an new scrol
  ...
  // NYI: need to redraw areas that the source of BitBlt was bad due t
  ...
  // this only works when scrolling in response t
  // a user event; Fl_Window::x/y_root() do not wor
  // on WIN32..
