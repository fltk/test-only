...
// Fl_Window itself is a subclass of this, and most of the even
...
// Hack: A single child is stored in the pointer to the array, whil
...
// Metrowerks CodeWarrior and others can't export the stati
...
extern Fl_Widget* fl_oldfocus; // set by Fl::focu

// For back-compatability, we must adjust all events sent to chil
...
static int send(Fl_Widget* o, int event) 
  ...
  case FL_ENTER
  case FL_DND_ENTER
    // Successful completion of FL_ENTER means the widget is now th
    // belowmouse widget, but only call Fl::belowmouse if the chil
...
static int navkey() 
  switch (Fl::event_key()) 
  case 0: // not an FL_KEYBOARD/FL_SHORTCUT even
    break
  case FL_Tab
    if (!Fl::event_state(FL_SHIFT)) return FL_Right
  case 0xfe20: // XK_ISO_Left_Ta
    return FL_Left
  case FL_Right
    return FL_Right
  case FL_Left
...
: Fl_Widget(X,Y,W,H,l) 
  ...
  savedfocus_ = 0
  resizable_ = this
  sizes_ = 0; // this is allocated when first resize() is don
  // Subclasses may want to construct child objects as part of thei
...
void Fl_Group::clear() 
  ...
  children_ = 0
  //array_ = 0; //dont do this, it will clobber old_array if only one chil
  savedfocus_ = 0
  resizable_ = this
  init_sizes()
...
void Fl_Group::insert(Fl_Widget &o, int index) 
  ...
  o.parent_ = this
  if (children_ == 0) { // use array pointer to point at single chil
    array_ = (Fl_Widget**)&o
  } else if (children_ == 1) { // go from 1 to 2 childre
    ...
    if (!(children_ & (children_-1))) // double number of childre
...
void Fl_Group::remove(Fl_Widget &o) 
  ...
  o.parent_ = 0
  children_--
  if (children_ == 1) { // go from 2 to 1 chil
    Fl_Widget *t = array_[!i]
    free((void*)array_)
    array_ = (Fl_Widget**)t
  } else if (children_ > 1) { // delete from arra
...
// Rather lame kludge here, I need to detect windows and ignore th
// changes to X,Y, since all children are relative to X,Y.  Tha
// is why I check type()

// sizes array stores the initial positions of widgets a
// left,right,top,bottom quads.  The first quad is the group, th
// second is the resizable (clipped to the group), and th
// rest are the children.  This is a convienent order for th
// algorithim.  If you change this be sure to fix Fl_Tile whic
...
short* Fl_Group::sizes() 
  if (!sizes_) 
    ...
    p[4] = p[0]; // init to the group's siz
    p[5] = p[1]
    p[6] = p[2]
    p[7] = p[3]
    Fl_Widget* r = resizable()
    if (r && r != this) { // then clip the resizable to i
      int t
