...
#define PREFERRED_GAP_SIZE 8
/* Initial size for the buffer gap (empty spac
in the buffer where text might be inserte
...
static int undoat;	// points after insertio
static int undocut;	// number of characters deleted ther
static int undoinsert;	// number of characters inserte
static int undoyankcut;	// length of valid contents of buffer, even if undocut=
...
/
** Create an empty text buffer of a pre-determined size (use this t
** avoid unnecessary re-allocation if you know exactly how much the buffe
** will need to hol
*
...
/
** Get the entire contents of a text buffer.  Memory is allocated to contai
...
/
** Replace the entire contents of the text buffe
*
...
/
** Return a copy of the text between "start" and "end" character position
** from text buffer "buf".  Positions start at 0, and the range does no
...
/
** Delete the characters between "start" and "end", and insert th
...
void Fl_Text_Buffer::copy( Fl_Text_Buffer *fromBuf, int fromStart
  ...
  /* Prepare the buffer to receive the new text.  If the new text fits i
     the current buffer, just move the gap (if necessary) to wher
     the text should be inserted.  If the new text is too large, reallocat
     the buffer with a gap large enough to accomodate the new text and 
...
/
** let the undo system know if we can undo change
*
...
/
** Insert "text" columnwise into buffer starting at displayed characte
** position "column" on the line beginning at "startPos".  Opens a rectangula
** space the width and height of "text", by moving all text to the right o
** "column" right.  If charsInserted and charsDeleted are not NULL, th
** number of characters inserted and deleted in the operation (beginnin
** at startPos) are returned in these argument
*
...
/
** Overlay "text" between displayed character positions "rectStart" an
** "rectEnd" on the line beginning at "startPos".  If charsInserted an
** charsDeleted are not NULL, the number of characters inserted and delete
...
** and "rectEnd", with "text".  If "text" is vertically longer than th
...
void Fl_Text_Buffer::replace_rectangular( int start, int end, int rectStart
  ...
  /* Make sure start and end refer to complete lines, since th
  ...
  /* If more lines will be deleted than inserted, pad the inserted tex
     with newlines to make it as long as the number of deleted lines.  Thi
     will indent all of the text to the right of the rectangle to the sam
     column.  If more lines will be inserted than deleted, insert extr
     lines in the buffer at the end of the rectangle to make room for th
...
/
** Remove a rectangular swath of characters between character positions star
...
/
** Clear a rectangular "hole" out of the buffer between character position
** start and end and horizontal displayed-character offsets rectStart an
** rectEnd
*
...
char * Fl_Text_Buffer::text_in_rectangle( int start, int end
  ...
  *outPtr = '\0'
  ...
  /* If necessary, realign the tabs in the selection as if the text wer
...
int Fl_Text_Buffer::selection_position( int *start, int *en
                                      ) 
...
/
** Add a callback routine to be called when the buffer is modifie
*
...
void Fl_Text_Buffer::remove_modify_callback( Fl_Text_Modify_Cb bufModifiedCB
  ...
  /* Allocate new lists for remaining callback procs and args (i
     any are left) *
  mNModifyProcs--
  if ( mNModifyProcs == 0 ) 
    mNModifyProcs = 0
...
/
** Get a character from the text buffer expanded into it's scree
** representation (which may be several characters for a tab or 
** control code).  Returns the number of characters written to "outStr"
** "indent" is the number of characters from the start of the lin
** for figuring tabs.  Output string is guranteed to be shorter o
** equal in length to FL_TEXT_MAX_EXP_CHAR_LE
*
...
/
** Expand a single character from the text buffer into it's scree
** representation (which may be several characters for a tab or 
...
/
** Return the length in displayed characters of character "c" expande
** for display (as discussed above in BufGetExpandedChar).  If th
** buffer for which the character width is being measured is doing nul
** substitution, nullSubsChar should be passed as that character (or nu
** to ignore)
*
...
/
** Count the number of displayed characters between buffer positio
** "lineStartPos" and "targetPos". (displayed characters are the character
** shown on the screen to represent characters in the buffer, where tabs an
...
/
** Count forward from buffer position "startPos" in displayed character
** (displayed characters are the characters shown on the screen to represen
...
/
** Find the position of the first character of the line "nLines" backward
** from "startPos" (not counting the character pointed to by "startpos" i
** that is a newline) in "buf".  nLines == 0 means find the beginning o
** the lin
*
...
/
** Search forwards in buffer for string "searchString", starting with th
...
/
** Search backwards in buffer for string "searchString", starting with th
...
/
** Search forwards in buffer for characters in "searchChars", startin
...
/
** Search backwards in buffer for characters in "searchChars", startin
...
/
** A horrible design flaw in NEdit (from the very start, before we knew tha
** NEdit would become so popular), is that it uses C NULL terminated string
** to hold text.  This means editing text containing NUL characters is no
** possible without special consideration.  Here is the special consideration
** The routines below maintain a special substitution-character which stand
** in for a null, and translates strings an buffers back and forth from/t
** the substituted form, figure out what to substitute, and figure ou
** when we're in over our heads and no translation is possible
*

/
** The primary routine for integrating new text into a text buffer wit
** substitution of another character for ascii nuls.  This substitutes nul
** characters in the string in preparation for being copied or replace
** into the buffer, and if neccessary, adjusts the buffer as well, in th
** event that the string contains the character it is currently using fo
** substitution.  Returns 0, if substitution is no longer possibl
...
int Fl_Text_Buffer::substitute_null_characters( char *string, int len ) 
  ...
  /* Does the string contain the null-substitute character?  If so, re
     histogram the buffer text to find a character which is ok in both th
     string and the buffer, and change the buffer's null-substitutio
  ...
  /* If the string contains null characters, substitute them with th
...
/
** Convert strings obtained from buffers which contain null characters, whic
** have been substituted for by a special substitution character, back t
** a null-containing string.  There is no time penalty for calling thi
...
/
** Create a pseudo-histogram of the characters in a string (don't actuall
** count, because we don't want overflow, just mark the character's presenc
...
/
** Search through ascii control characters in histogram in order of leas
** likelihood of use, find an unused character to use as a stand-in for 
** null.  If the character set is full (no available characters outside o
** the printable set, return the null character
*
static char chooseNullSubsChar( char hist[ 256 ] ) 
#define N_REPLACEMENTS 2
...
/
** Internal (non-redisplaying) version of BufInsert.  Returns the length o
** text inserted (this is just strlen(text), however this calculation can b
** expensive and the length will be required by any caller who will continu
** on to call redisplay).  pos must be contiguous with the existing text i
...
/
** Internal (non-redisplaying) version of BufRemove.  Removes the content
** of the buffer between start and end (and moves the gap to the site o
** the delete)
*
...
/
** Insert a column of text without calling the modify callbacks.  Note tha
** in some pathological cases, inserting can actually decrease the size o
** the buffer because of spaces being coalesced into tabs.  "nDeleted" an
** "nInserted" return the number of characters deleted and inserted beginnin
** at the start of the line containing "startPos".  "endPos" returns buffe
** position of the lower left edge of the inserted column (as a hint fo
...
void Fl_Text_Buffer::insert_column_( int column, int startPos, const char *insText
  ...
  /* Allocate a buffer for the replacement string large enough to hol
     possibly expanded tabs in both the inserted text and the replace
     area, as well as per line: 1) an additional 2*FL_TEXT_MAX_EXP_CHAR_LE
     characters for padding where tabs and control characters cross th
     column of the selection, 2) up to "column" additional spaces pe
     line for padding out to the position of "column", 3) padding u
     to the width of the inserted text if that must be padded to alig
     the text beyond the inserted column.  (Space for additiona
     newlines if the inserted text extends beyond the end of the buffe
  ...
  /* Loop over all lines in the buffer between start and end removing th
     text between rectStart and rectEnd and padding appropriately.  Tri
     trailing space from line (whitespace at the ends of lines otherwis
...
/
** Delete a rectangle of text without calling the modify callbacks.  Return
** the number of characters replacing those between start and end.  Note tha
** in some pathological cases, deleting can actually increase the size o
** the buffer because of tab expansions.  "endPos" returns the buffer positio
** of the point in the last line where the text was removed (as a hint fo
...
void Fl_Text_Buffer::remove_rectangular_( int start, int end, int rectStart
  ...
  /* allocate a buffer for the replacement string large enough to hol
     possibly expanded tabs as well as an additional  FL_TEXT_MAX_EXP_CHAR_LEN * 
     characters per line for padding where tabs and control characters cros
  ...
  /* loop over all lines in the buffer between start and end removin
...
** "nDeleted" and "nInserted" return the number of characters deleted an
** inserted beginning at the start of the line containing "startPos"
** "endPos" returns buffer position of the lower left edge of the inserte
...
void Fl_Text_Buffer::overlay_rectangular_(int startPos, int rectStart
  ...
  /* Allocate a buffer for the replacement string large enough to hol
     ...
     an additional 2*FL_TEXT_MAX_EXP_CHAR_LEN characters for padding where tab
     and control characters cross the column of the selection, 2) up t
     "column" additional spaces per line for padding out to the positio
     of "column", 3) padding up to the width of the inserted text if tha
     must be padded to align the text beyond the inserted column.  (Spac
     for additional newlines if the inserted text extends beyond the en
  ...
  /* Loop over all lines in the buffer between start and end overlaying th
     text between rectStart and rectEnd and padding appropriately.  Tri
     trailing space from line (whitespace at the ends of lines otherwis
     tends to multiply, since additional padding is added to maintain it *
  outPtr = outStr
...
/
** Insert characters from single-line string "insLine" in single-line strin
...
** returns the number of characters from the beginning of the string t
** the right edge of the inserted text (as a hint for routines which nee
...
static void insertColInLine( const char *line, char *insLine, int column, int insWidth
  ...
  /* If "column" falls in the middle of a character, and the character is 
     tab, leave it off and leave the indent short and it will get padde
     later.  If it's a control character, insert it and adjust inden
     accordingly. *
  ...
  /* Copy the text from "insLine" (if any), recalculating the tabs as i
  ...
  /* Pad out to column + width of inserted text + (additional origina
...
/
** Remove characters in single-line string "line" between displayed position
** "rectStart" and "rectEnd", and write the result to "outStr", which i
** assumed to be large enough to hold the returned string.  Note that i
** certain cases, it is possible for the string to get longer due t
** expansion of tabs.  "endOffset" returns the number of characters fro
** the beginning of the string to the point where the characters wer
...
static void deleteRectFromLine( const char *line, int rectStart, int rectEnd
  ...
  /* fill in any space left by removed tabs or control character
  ...
  /* Copy the rest of the line.  If the indentation has changed, preserv
     the position of non-whitespace characters by converting tabs t
...
/
** Overlay characters from single-line string "insLine" on single-line strin
...
static void overlayRectInLine( const char *line, char *insLine, int rectStart
  ...
  /* If "rectStart" falls in the middle of a character, and the characte
     is a tab, leave it off and leave the outIndent short and it will ge
     padded later.  If it's a control character, insert it and adjus
  ...
  /* Pad out to rectEnd + (additional original offse
...
/
** Return true if position "pos" with indentation "dispIndex" is i
...
void Fl_Text_Buffer::replace_selection_( Fl_Text_Selection *sel, const char *s ) 
  ...
  /* Unselect (happens automatically in BufReplace, but BufReplaceRec
...
/
** Call the stored modify callback procedure(s) for this buffer to update th
...
/
** Call the stored redisplay procedure(s) for this buffer to update th
...
void Fl_Text_Buffer::redisplay_selection( Fl_Text_Selection *oldSelection
  ...
  /* If either selection is rectangular, add an additional character t
     the end of the selection to request the redraw routines to wipe ou
  ...
  /* If the old or new selection is unselected, just redisplay th
  ...
  /* If the selection changed from normal to rectangular or visa versa, o
  ...
  /* If the selections are non-contiguous, do two separate update
     and return *
  ...
  /* Otherwise, separate into 3 separate regions: ch1, and ch2 (the tw
...
/
** Update all of the selections in "buf" for changes in the buffer's tex
*
...
/
** Update an individual selection for changes in the corresponding tex
*
...
/
** Search forwards in buffer "buf" for character "searchChar", startin
** with the character "startPos", and returning the result in "foundPos
** returns 1 if found, 0 if not.  (The difference between this an
** BufSearchForward is that it's optimized for single characters.  Th
** overall performance of the text widget is dependent on its ability t
...
/
** Search backwards in buffer "buf" for character "searchChar", startin
** with the character BEFORE "startPos", returning the result in "foundPos
** returns 1 if found, 0 if not.  (The difference between this an
** BufSearchBackward is that it's optimized for single characters.  Th
** overall performance of the text widget is dependent on its ability t
** count lines quickly, hence searching for a single character: newline
*
...
** and return the copy as the function value, and the length of the line i
** "lineLen
*
static char *copyLine( const char *text, int *lineLen ) 
  int len = 0
...
/
** Find the first and last character position in a line within a rectangula
** selection (for copying).  Includes tabs which cross rectStart, but no
** control characters which do so.  Leaves off tabs which cross rectEnd
*
** Technically, the calling routine should convert tab characters whic
** cross the right boundary of the selection to spaces which line up wit
** the edge of the selection.  Unfortunately, the additional memor
** management required in the parent routine to allow for the change
** in string size is not worth all the extra work just for a couple o
** shifted characters, so if a tab protrudes, just lop it off and hop
** that there are other characters in the selection to establish the righ
...
/
** Adjust the space and tab characters from string "text" so that non-whit
** characters remain stationary when the text is shifted from starting a
** "origIndent" to starting at "newIndent".  Returns an allocated strin
...
static char *realignTabs( const char *text, int origIndent, int newIndent
  ...
  /* If the tab settings are not the same, brutally convert tabs t
...
/
** Expand tabs to spaces for a block of text.  The additional paramete
** "startIndent" if nonzero, indicates that the text is a rectangular selectio
...
/
** Convert sequences of spaces into tabs.  The threshold for conversion i
** when 3 or more spaces can be converted into a single tab, this avoid
