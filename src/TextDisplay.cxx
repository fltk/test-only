...
// can this API be included in the real FLTK?? (pretty plz...)
...
TextDisplay::TextDisplay(int X, int Y, int W, int H,  const char* l)
  ...
  box(THIN_DOWN_BOX);
  textsize((int)Widget::default_style->labelsize());
  textfont(Widget::default_style->labelfont());
  ...
  scrollbar_width(16);
  scrollbar_align(ALIGN_BOTTOM|ALIGN_RIGHT);
...
*/
//void TextDisplay::resize(int X, int Y, int W, int H) {
void TextDisplay::layout() {
  if (!(layout_damage()&~LAYOUT_XY)) {Widget::layout(); return;}

  text_area.set(0,0,w(),h());
  box()->inset(text_area);
  text_area.move_x(LEFT_MARGIN);
  text_area.move_y(TOP_MARGIN);
  text_area.move_r(-RIGHT_MARGIN);
  text_area.move_b(-BOTTOM_MARGIN);

  // Find the new maximum font height for this text display:
  mMaxsize = int(textsize()+leading());
  for (int i = 0; i < mNStyles; i++) {
    mMaxsize = max(mMaxsize, int(mStyleTable[i].size+leading()));
  }

  // try without scrollbars first
  mVScrollBar->clear_visible();

  // horizontal one only turns off if they change the width:
  if (layout_damage()&LAYOUT_W) {
    mHScrollBar->clear_visible();
  } else if (mHScrollBar->visible()) {
    text_area.move_b(-scrollbar_width());
  }
  ...
  for (int again = 1; again;) {
    ...
    // Turn on vertical scrollbar if needed:
    if (!mVScrollBar->visible() &&
        (scrollbar_align() & (ALIGN_LEFT|ALIGN_RIGHT)) &&
        mNBufferLines >= mNVisibleLines - 1) {
      mVScrollBar->set_visible();
      text_area.move_r(-scrollbar_width());
    }
    /*
      Decide if the horizontal scroll bar needs to be visible.

      Originally nedit turned it on if any of the visible lines
      were too long, but it always reserved the space for the
      bars.Since the FLTK widget dynamically allocates the space
      for the scrollbar and rearranges the widget to make room for
      it, this would create a very visually displeasing "bounce"
      effect when the vertical scrollbar is dragged.  Trust me, I
      tried it and it looks really bad.

      This one turns it on the moment it sees a line that is too
      big and does not turn it off unless the user resizes
      horizontally.

      Other algorithims tried were to always turn it on if the
      vertical one was on.

      Another alternative would be to keep track of what the
      longest line in the entire buffer is and base the scrollbar
      on that. I suspect this is what most toolkits do. I didn't
      do this because I didn't see any easy way to do that using
      the nedit code and this could involve a lengthy calculation
      for large buffers.  If an efficient and non-costly way of
      doing this can be found, this might be a way to go.
    */
    if (!mHScrollBar->visible() &&
        (scrollbar_align() & (ALIGN_TOP|ALIGN_BOTTOM)) &&
  ...
  // adjust the location of the text area and scrollbars, now that
  // we know the aligment and visibility of them:
  int full_y = text_area.y()-TOP_MARGIN;
  int full_h = text_area.h()+(TOP_MARGIN+BOTTOM_MARGIN);
  if (mHScrollBar->visible()) {
    if (scrollbar_align() & ALIGN_TOP) {
      mHScrollBar->resize(text_area.x()-LEFT_MARGIN,
                          ...
                          scrollbar_width());
      text_area.y(text_area.y()+scrollbar_width());
    } else {
      mHScrollBar->resize(text_area.x()-LEFT_MARGIN,
                          text_area.b()+BOTTOM_MARGIN,
                          text_area.w()+(LEFT_MARGIN+RIGHT_MARGIN),
                          scrollbar_width());
    }
    full_h += scrollbar_width();
  }

  if (mVScrollBar->visible()) {
    if (scrollbar_align() & ALIGN_LEFT) {
      mVScrollBar->resize(text_area.x()-LEFT_MARGIN,
                          full_y,
                          scrollbar_width(),
                          full_h);
      text_area.x(text_area.x()+scrollbar_width());
    } else {
      mVScrollBar->resize(text_area.r()+RIGHT_MARGIN,
                          full_y,
                          scrollbar_width(),
                          full_h);
    }
...
void TextDisplay::draw_text( int left, int top, int width, int height) {
  ...
  push_clip(Rectangle( left, top, width, height));
...
void TextDisplay::show_cursor(int b) {
  mCursorOn = b;
  redisplay_range(mCursorPos - 1, mCursorPos + 1);
}
...
void TextDisplay::wrap_mode(int wrap, int wrapMargin) {
...
int TextDisplay::position_to_xy( int pos, int* X, int* Y) {
...
*/
int TextDisplay::position_to_linecol( int pos, int* lineNum, int* column) {
  int retVal;
...
*/
int TextDisplay::in_selection( int X, int Y) {
...
#if 0 //def DEBUG
  printf("TextDisplay::count_line(startPos=%d, endPos=%d, startPosIsLineStart=%d\n",
...
void TextDisplay::next_word() {
  ...
  while (pos < buffer()->length() && !isseparator(buffer()->character(pos))) {
    pos++;
  }
  while (pos < buffer()->length() && isseparator(buffer()->character(pos))) {
    pos++;
...
void TextDisplay::previous_word() {
  ...
  if (pos==0) return;
  pos--;
  while (pos && isseparator(buffer()->character(pos))) {
    pos--;
  }
  while (pos && !isseparator(buffer()->character(pos))) {
    pos--;
  }
  if (isseparator(buffer()->character(pos))) pos++;
...
*/
void TextDisplay::maintain_absolute_top_line_number(int state) {
...
*/
int TextDisplay::position_to_line( int pos, int *lineNum) {
  int i;
    ...
    if (empty_vlines()) {
      if (mLastChar < mBuffer->length()) {
        if (!position_to_line( mLastChar, lineNum)) {
          //trace("TextDisplay::position_to_line(): Consistency check ptvl failed");
          return 0;
        }
...
void TextDisplay::draw_vline(int visLineNum, int leftClip, int rightClip,
  ...
  if (stdCharWidth <= 0) {
    //trace("TextDisplay::draw_vline(): bad font measurement");
    free((void *)lineStr);
    return;
  }
  ...
  for ( charIndex = 0; ; charIndex++) {
    const char *currStr = lineStr + charIndex;
    const char *nextStr = charIndex < lineLen ?
      utf8fwd(currStr+1, currStr, lineStr+lineLen) : lineStr+1;
    bool utf8 = false;
    if ((nextStr - currStr) <= 1)
  ...
  for ( charIndex = startIndex; charIndex < rightCharIndex; charIndex++) {
    const char *currStr = lineStr + charIndex;
    const char *nextStr = charIndex < lineLen ?
      utf8fwd(currStr+1, currStr, lineStr+rightCharIndex) : lineStr+1;
    bool utf8 = false;
    if ((nextStr - currStr) <= 1)
      charLen = charIndex >= lineLen ? 1 :
...
void TextDisplay::clear_rect( int style, int X, int Y,
  ...
  if (focus() != this) {
    if (style & (HIGHLIGHT_MASK | PRIMARY_MASK)) {
      setcolor(lerp(color(), selection_color(), 0.5f));
    } else {
      setcolor(color());
    }
    //setcolor(MAGENTA);
    fillrect(Rectangle( X, Y, width, height));
  } else if (style & HIGHLIGHT_MASK) {
    setcolor( contrast(textcolor(), color()));
    fillrect(Rectangle( X, Y, width, height));
...
int TextDisplay::position_style( int lineStartPos,
  ...
  TextBuffer * buf = mBuffer;
  TextBuffer *styleBuf = mStyleBuffer;
  int pos, style = 0;
...
#if 0 //def DEBUG
  printf("TextDisplay::update_v_scrollbar():\n"
...
*/
int TextDisplay::measure_proportional_character(char c, int colNum, int pos) {
  int charLen, style;
...
void TextDisplay::extend_range_for_styles( int *startpos, int *endpos) {
  TextSelection * sel = mStyleBuffer->primary_selection();
  int extended = 0;
...
void TextDisplay::draw(void) {
  ...
  else if (damage() & (DAMAGE_SCROLL | DAMAGE_EXPOSE)) {
    //trace("blanking previous cursor extrusions at Y: %d\n", mCursorOldY);
    // CET - FIXME - save old cursor position instead and just draw side needed?
    push_clip(Rectangle(text_area.x()-LEFT_MARGIN,
                        text_area.y(),
                        text_area.w()+LEFT_MARGIN+RIGHT_MARGIN,
                        text_area.h()));
    setcolor(color());
    fillrect(Rectangle(text_area.x()-LEFT_MARGIN, mCursorOldY,
                       LEFT_MARGIN, mMaxsize));
    fillrect(Rectangle(text_area.x()+text_area.w(), mCursorOldY,
  ...
  if (damage() & (DAMAGE_ALL | DAMAGE_CHILD)) {
    mVScrollBar->set_damage(DAMAGE_ALL);
    mHScrollBar->set_damage(DAMAGE_ALL);
  }
  ...
  if (damage() & (DAMAGE_ALL | DAMAGE_EXPOSE)) {
    ...
    push_clip(Rectangle(text_area.x(), text_area.y(),
                        text_area.w(), text_area.h()));
    //trace("drawing text from %d to %d\n", damage_range1_start, damage_range1_end);
  ...
  if (damage() & (DAMAGE_ALL | DAMAGE_SCROLL | DAMAGE_EXPOSE)
      && !buffer()->primary_selection()->selected() &&
      mCursorOn && focus() == this) {
    push_clip(Rectangle(text_area.x()-LEFT_MARGIN,
                        text_area.y(),
                        text_area.w()+LEFT_MARGIN+RIGHT_MARGIN,
                        text_area.h()));
    ...
    //trace("drew cursor at pos: %d (%d,%d)\n", mCursorPos, X, Y);
    mCursorOldY = Y;
    pop_clip();
  }
  pop_clip();
