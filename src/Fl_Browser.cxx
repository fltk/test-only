...
// I modified this from the original Forms data to use a linked lis
// so that the number of items in the browser and size of those item
// is unlimited.  The only problem is that the old browser used a
// index number to identify a line, and it is slow to convert from/t
// a pointer.  I use a cache of the last match to try to speed thi
// up

// Also added the ability to "hide" a line.  This set's it's height t
...
int Fl_Browser::item_height(void* lv) const 
  ...
  if (!l->txt[0]) 
    ...
    for (char* str = l->txt; str && *str; str++) 
      Fl_Font font = textfont(); // default fon
      int tsize = textsize(); // default siz
	...
	case 'B'
	case 'C': strtol(str, &str, 10); break;// skip a color numbe
...
int Fl_Browser::item_width(void* v) const 
  ...
  while (*i) { // add up all tab-seperated field
    char* e
    e = strchr(str, column_char())
    if (!e) break; // last one occupied by tex
    str = e+1
    ww += *i++
...
void Fl_Browser::item_draw(void* v, int X, int Y, int W, int H) const 
  ...
  while (W > 6) {	// do each tab-seperated fiel
    int w1 = W;	// width for this fiel
    char* e = 0; // pointer to end of field or null if non
    if (*i) { // find end of field and temporarily replace with 
      e = strchr(str, column_char())
      if (e) {*e = 0; w1 = *i++;
  ...
  BREAK
    ...
    *e = column_char(); // put the seperator bac
    X += w1
    W -= w1
    str = e+1
...
void Fl_Browser::swap(FL_BLINE *a, FL_BLINE *b) 
  ...
  } else if ( a->prev == b ) {		// B ADJACENT TO 
  ...
  // Disable cache -- we played around with position
  cacheline = 0
  // Redraw modified line
  redraw_line(a)
  redraw_line(b)
