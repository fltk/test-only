...
void Fl_GDI_Printer::sty(int style, int width, char *dashes, int vex)
  ...
  if(!(width))
    width=1
    if(!style && !dashes
      style |= FL_CAP_SQUARE | FL_JOIN_MITER; //adjustment for system drawing
  
  if(!dashes || !(*dashes))
    if((style & 0xff) && ((style & 0xf00)==FL_CAP_SQUARE)) // square caps do not seem to work with dashes, not sure wh
