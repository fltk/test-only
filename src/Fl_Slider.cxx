...
//	p = position, first line displaye
//	w = window, number of lines displaye
//	t = top, number of first lin
//	l = length, total number of line
  step(1, 1)
  if (p+W > t+l) l = p+W-t
...
// All slider interaction is done as though the slider ranges fro
// zero to one, and the left (bottom) edge of the slider is at th
// given position.  Since when the slider is all the way to th
// right (top) the left (bottom) edge is not all the way over, 
...
void Fl_Slider::draw(int X, int Y, int W, int H) 
  ...
  if (!box1) {
    box1 = Fl::box(Fl::box_index(box())&-2); if (!box1) box1 = FL_UP_BOX;
  if (type() == FL_VERT_NICE_SLIDER) 
