...
//	Fl_Menu_Bar, Fl_Menu_Button, Fl_Choic
// This provides storage for a menu item, functions to add/modify/delet
// items, and a call for when the user picks a menu item

// More code in Fl_Menu_add.cx
...
// Set 'pathname' of specified menuite
...
int Fl_Menu_::item_pathname(char *name, int namelen, const Fl_Menu_Item *finditem) const 
    ...
    *name = '\0'
    return(-1);						// item not foun


// FIND MENU ITEM INDEX, GIVEN MENU PATHNAM
//     eg. "Edit/Copy
...
Fl_Menu_::find_item(const char *name
  ...
  for ( int t=0; t < size(); t++ ) 
	...
	continue
      ...
      // IT'S A MENU ITE
      char itempath[1024];	// eg. Edit/Cop
...
// turn on one of a set of radio button
void Fl_Menu_Item::setonly() 
  flags |= FL_MENU_RADIO | FL_MENU_VALUE
  Fl_Menu_Item* j
  for (j = this; ; ) {	// go dow
    if (j->flags & FL_MENU_DIVIDER) break; // stop on divider line
    j++
    if (!j->text || !j->radio()) break; // stop after grou
    j->clear()
...
: Fl_Widget(X,Y,W,H,l) 
  ...
  textcolor(FL_FOREGROUND_COLOR); //was: FL_BLAC
  down_box(FL_NO_BOX)
...
void Fl_Menu_::copy(const Fl_Menu_Item* m, void* ud) 
  ...
  menu(newMenu)
  alloc = 1; // make destructor free array, but not string
