...
void Fl_Window::hotspot(int X, int Y, int offscreen) 
  ...
  // If offscreen is 0 (the default), make sure that the windo
  // stays on the screen, if possible
  if (!offscreen) 
#if defined(WIN32) || defined(__APPLE__
    // These will be used by reference, so we must passed different variable
    int bt,bx,by
    x(X);y(Y)
...
#els
    if (border()) 
      // Ensure border is on screen; these values are generic enoug
