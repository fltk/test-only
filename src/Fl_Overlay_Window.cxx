...
// A window using double-buffering and able to draw an overla
// on top of that.  Uses the hardware to draw the overlay i
...
void Fl_Overlay_Window::flush() 
  ...
  if (overlay_ && overlay_ != this && overlay_->shown()) 
    // all drawing to windows hidden by overlay windows is ignored, fix thi
...
extern uchar fl_overlay; // changes how fl_color(x) work
...
void Fl_Overlay_Window::redraw_overlay() 
  if (!fl_display) return; // this prevents fluid -c from opening displa
  if (!overlay_) 
    if (can_do_overlay()) 
