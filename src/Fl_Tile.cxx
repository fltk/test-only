...
// Group of 2 or 4 "tiles" that can be resized by dragging borde
...
void Fl_Tile::position(int oix, int oiy, int newx, int newy) 
  ...
  short* p = sizes()
  p += 8; // skip group & resizable's saved siz
...
#define GRABAREA 
  ...
  case FL_DRAG
    ...
    // if (damage()) return 1; // don't fall behin
  case FL_RELEASE: 
    if (!sdrag) return 0; // should not happe
