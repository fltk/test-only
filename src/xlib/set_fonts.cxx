...
// "$Id$
/
// Xlib X11 font utilities for the Fast Light Tool Kit (FLTK)
/
...
#if USE_XF
#  include "set_fonts_xft.cxx
#els

// This function fills in the fltk font table with all the fonts tha
// are found on the X server.  It tries to place the fonts into familie
// and to sort them so the first 4 in a family are normal, bold, italic
// and bold italic

// Standard X fonts are matched by a pattern that is always o
// this form, and this pattern is put in the table
// "-*-family-weight-slant-width1-style-*-registry-encoding

// Non-standard font names (those not starting with '-') are matche
// by a pattern of the form "prefix*suffix", where the '*' is wher
// fltk thinks the point size is, or by the actual font name if n
// point size is found

// Fltk knows how to pull an "attribute" out of a font name, such a
// bold or italic, by matching known x font field values.  All word
// that don't match a known attribute are combined into the "name
// of the font.  Names are compared before attributes for sorting, thi
// makes the bold and plain version of a font come out next to eac
// other despite the poor X font naming scheme

// By default fl_set_fonts() only does iso8859-1 encoded fonts.  You ca
// do all normal X fonts by passing "-*" or every possible font with "*"

// Fl::set_font will take strings other than the ones this store
// and can identify any font on X that way.  You may want to write you
// own system of font management and not use this code

// turn word N of a X font name into either some attribute bit
// (right now 0, FL_BOLD, or FL_ITALIC), or into -1 indicating tha
...
// Bug: older versions calculated the value for *ap as a side effect o
// making the name, and then forgot about it. To avoid having to chang
// the header files I decided to store this value in the last characte
// of the font name array
#define ENDOFBUFFER 127 // sizeof(Fl_Font.fontname)-
...
Fl_Font Fl::set_fonts(const char* xstarname) 
  if (fl_free_font > FL_FREE_FONT) // already been her
  ...
  for (int i=0; i<xlistsize;) 
	...
	// if so, set the list of x fonts, since we have it anywa
...
int Fl::get_font_sizes(Fl_Font fnum, int*& sizep) 
  ...
  if (!s->name) s = fl_fonts; // empty slot in table, use entry 
  if (!s->xlist) 
    fl_open_display()
