...
fl_read_image(uchar *p,		// I - Pixel buffer or NULL to allocat
  ...
  XImage	*image;		// Captured imag
  int		i, maxindex;	// Looping var
  int           x, y;		// Current X & Y in imag
  int		d;		// Depth of imag
  unsigned char *line,		// Array to hold image ro
		*line_ptr;	// Pointer to current line imag
  unsigned char	*pixel;		// Current color valu
  ...
  /
  // Under X11 we have the option of the XGetImage() interface or SGI'
  // ReadDisplay extension which does all of the really hard work fo
  // us..
  /
