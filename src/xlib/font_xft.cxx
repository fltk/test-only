...
/
// This font code only requires libXft to work.  Contrary to popula
// belief there is no need to have FreeType, or the Xrender extensio
// available to use this code.  You will just get normal Xlib font
// (Xft calls them "core" fonts) The Xft algorithms for choosin
// these is about as good as the FLTK ones (I hope to fix it so it i
// exactly as good...), plus it can cache it's results and share the
// between programs, so using this should be a win in all cases. Als
// it should be obvious by comparing this file and fl_font_x.cxx tha
// it is a lot easier to program with Xft than with Xlib
/
// Also, Xft supports UTF-8 text rendering directly, which will allo
// us to support UTF-8 on all platforms more easily
/
// To actually get antialiasing you need the following
/
//     1. You have XFree86 
//     2. You have the XRender extensio
//     3. Your X device driver supports the render extensio
//     4. You have libXf
//     5. Your libXft has FreeType2 support compiled i
//     6. You have the FreeType2 librar
/
// Distributions that have XFree86 4.0.3 or later should have all of this..
/
// Unlike some other Xft packages, I tried to keep this simple and no
// to work around the current problems in Xft by making the "patterns
// complicated. I belive doing this defeats our ability to improve Xf
// itself. You should edit the ~/.xftconfig file to "fix" things, ther
...
void fl_font(int fnum, int size) 
  ...
  Fl_FontSize* f
  // search the fontsizes we have generated alread
...
static XftFont* fontopen(const char* name, bool core) 
  ...
  // may be efficient, but this is non-obviou
  switch (*name++) 
...
#if USE_OVERLA
// Currently Xft does not work with colormapped visuals, so this probabl
...
// For some reason Xft produces errors if you destroy a window whose i
// still exists in an XftDraw structure. It would be nice if this is no
...
void Fl_Xlib_Display::draw(const char *str, int n, int x, int y) 
#if USE_OVERLA
  ...
  // Use fltk's color allocator, copy the results to match wha
