...
Fl_FontSize::Fl_FontSize(const char* name) 
  ...
  if (!font) 
    ...
    font = XLoadQueryFont(fl_display, "fixed"); // if fixed fails we cras
...
static Fl_FontSize* find(int fnum, int size) 
  ...
  if (!s->name) s = fl_fonts; // use font 0 if still undefine
  Fl_FontSize* f
  ...
  char* name = s->xlist[0]; int ptsize = 0;	// best one found so fa
  int matchedlength = 32767
  char namebuffer[1024];	// holds scalable font nam
  ...
  for (int n=0; n < m; n++) 
    ...
    if (fl_correct_encoding(thisname)) 
      if (!found_encoding) ptsize = 0; // force it to choose thi
      found_encoding = 1
    } else 
    ...
    if (thissize == size && thislength < matchedlength) 
      ...
      name = namebuffer
      ptsize = size
    } else if (!ptsize ||	// no fonts ye
	       thissize < ptsize && ptsize > size || // current font too bi
	       thissize > ptsize && thissize <= size // current too smal
      ) 
...
void Fl_Xlib_Display::font(int fnum, int size) 
