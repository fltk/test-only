...
// "$Id$
/
// Xlib image drawing routines for the Fast Light Tool Kit (FLTK)
/
...
// PsuedoColor visuals must have 8 bits_per_pixel (although the dept
// may be less than 8).  This is the only limitation that affects an
// modern X displays, you can't use 12 or 16 bit colormaps

// The mask bits in TrueColor visuals for each color ar
// contiguous and have at least one bit of each color.  Thi
// is not checked for
...
static XImage xi;	// template used to pass info to 
...
static int dir;		// direction-alternato
static int ri,gi,bi;	// saved error-diffusion valu

#  if USE_COLORMA
///////////////////////////////////////////////////////////////
// 8-bit converter with error diffusio
...
///////////////////////////////////////////////////////////////
// 16 bit TrueColor converters with error diffusio
// Cray computers have no 16-bit type, so we use character pointer
// (which may be slow

#  ifdef U1
#    define OUTTYPE U1
#    define OUTSIZE 
#    define OUTASSIGN(v) *t = 
#  els
#    define OUTTYPE ucha
#    define OUTSIZE 
...
#    define STORETYPE U3
#    define INNARDS32(f) 
  U32 *t = (U32*)to; for (; w--; from += delta) *t++ = 
#  endi
...
static void figure_out_visual() 
  ...
  fl_xpixel(FL_BLACK); // setup fl_redmask, etc, in fl_color.cx
  fl_xpixel(FL_WHITE); // also make sure white is allocate
...
//i.bitmap_pad = 8
  ...
  if (xi.bits_per_pixel & 7) bytes_per_pixel = 0; // produce fatal erro
...
#  endi
  ...
  case 2
    // All 16-bit TrueColor visuals are supported on any machine wit
...
static void innards(const uchar *buf, int X, int Y, int W, int H
  ...
  // See if the data is already in the right format.  Unfortunatel
  // some 32-bit x servers (XFree86) care about the unknown 8 bit
  // and they must be zero.  I can't confirm this for user-supplie
  // data, so the 32-bit shortcut is disabled..
  // This can set bytes_per_line negative if image is bottom-to-to
  // I tested it on Linux, but it may fail on other Xlib implementations
  if (buf && 
#  if 0	// set this to 1 to allow 32-bit shortcu
      delta == 4 &
#    if WORDS_BIGENDIA
      conv == rgbx_converte
#    els
...
#  endi
    ...
    int blocking = h
    static STORETYPE *buffer;	// our storage, always word aligne
...
void Fl_Xlib_Display::draw_image(const uchar* buf, int x, int y, int w, int h, int d, int l)
  innards(buf,x,y,w,h,d,l,(d<3&&d>-3),0,0)

void Fl_Xlib_Display::draw_image(Fl_Draw_Image_Cb cb, void* data
		   int x, int y, int w, int h,int d) 
  innards(0,x,y,w,h,d,0,(d<3&&d>-3),cb,data)

void Fl_Xlib_Display::draw_image_mono(const uchar* buf, int x, int y, int w, int h, int d, int l)
  innards(buf,x,y,w,h,d,l,1,0,0)

void Fl_Xlib_Display::draw_image_mono(Fl_Draw_Image_Cb cb, void* data
		   int x, int y, int w, int h,int d) 
  innards(0,x,y,w,h,d,0,1,cb,data)


void Fl_Xlib_Display::rectf(int x, int y, int w, int h, uchar r, uchar g, uchar b) 
