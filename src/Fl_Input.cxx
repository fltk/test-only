...
// This is the "user interface", it decodes user actions into what t
// do to the text.  See also Fl_Input_.cxx, where the text is actuall
// manipulated (and some ui, in particular the mouse, is done...)
// In theory you can replace this code with another subclass to chang
// the keybindings
...
// If you define this symbol as zero you will get the peculiar flt
// behavior where moving off the end of an input field will move th
...
int Fl_Input::handle_key() 
  ...
  case FL_Right
    ascii = ctrl('F'); break
  case FL_Page_Up
    repeat_num=5; //temporary hac
                  //TODO: find number of lines in window and use it instead 
  case FL_Up
    ascii = ctrl('P'); break
  case FL_Page_Down
    repeat_num=5; //temporary hac
  ...
  case FL_KP_Enter
    if (when() & FL_WHEN_ENTER_KEY) 
      ...
      return 0;	// reserved for shortcut
  case FL_Tab
...
int Fl_Input::handle(int event) 
  ...
  case FL_FOCUS
    ...
    case 0xfe20: // XK_ISO_Left_Ta
      ...
      break
    default
      position(position(),mark());// turns off the saved up/down arrow positio
      break
  ...
  case FL_PUSH
	...
	// user clicked in the selection, may be trying to dra
	drag_start = newpos
	return 1
  ...
  case FL_DRAG
    if (Fl::dnd_text_ops()) 
      if (drag_start >= 0) 
	if (Fl::event_is_click()) return 1; // debounce the mous
  ...
  case FL_RELEASE
    if (Fl::event_button() == 2) 
      Fl::event_is_click(0); // stop double click from picking a wor
      Fl::paste(*this, 0)
      ...
      // user double or triple clicked to select word or whole tex
      copy(0)
    
    // For output widgets, do the callback so the app knows the use
    // did something with the mouse..
    if (readonly()) do_callback()
    return 1

  case FL_DND_ENTER
    Fl::belowmouse(this); // send the leave events firs
    dnd_save_position = position()
    dnd_save_mark = mark()
