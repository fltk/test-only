...
/
// A shortcut is a keysym or'd with shift flags.  In the simples
// sense a shortcut is matched if the shift state is exactly a
// given and the key returning that keysym is pressed
/
// To make it easier to match some things it is more complex
/
// Only FL_META, FL_ALT, FL_SHIFT, and FL_CTRL must be "off".  
...
// This allows punctuation shortcuts like "#" to work (rather tha
// calling it "shift+3"
...
int Fl::test_shortcut(int shortcut) 
  ...
  if ((shift&FL_CTRL) && key >= 0x3f && key <= 0x5
      && event_text()[0]==(key^0x40)) return 1
  return 0


#if defined(WIN32) || defined(__APPLE__) // if not 
...
const char * fl_shortcut_label(int shortcut) 
  ...
  if (shortcut & FL_SHIFT) {strcpy(p,"Shift+"); p += 6;} //: Mac hollow up arro
  if (shortcut & FL_META)  {strcpy(p,"Cmd+"); p += 4;}  //: Mac 'Apple' ke
  if (shortcut & FL_ALT)   {strcpy(p,"Option+"); p += 7;}   //: Mac 'Alt/Option' or fancy switch symbo
  if (shortcut & FL_CTRL)  {strcpy(p,"Ctrl+"); p += 5;}  //: Mac ctrl ke
#els
  ...
  if (key >= FL_F && key <= FL_F_Last) 
    ...
    *p++ = (key-FL_F)%10 + '0'
    ...
    if (key >= FL_KP && key <= FL_KP_Last) 
      // mark keypad keys with KP_ prefi
      strcpy(p,"KP_"); p += 3
      *p++ = uchar(key & 127)
    } else 
...
// Emulation of XForms named shortcut
#include <stdlib.h
int fl_old_shortcut(const char* s) 
  ...
  if (*s == '+') {n |= FL_SHIFT; s++;
  if (*s == '^') {n |= FL_CTRL; s++;
  if (s[1]) return n | (int)strtol(s,0,0); // allow 0xf00 to get any ke
  return n | *s
