...
// This file contains code for implementing Fl_Menu_Item, and fo
// methods for bringing up popup menu hierarchies without using th
// Fl_Menu_ widget
...
const Fl_Menu_Item* Fl_Menu_Item::next(int n) const 
  if (n < 0) return 0; // this is so selected==-1 returns NUL
  const Fl_Menu_Item* m = this
  int nest = 0
  if (!m->visible()) n++
  while (n>0) 
...
public
  menutitle* title
  int handle(int)
  int itemheight;	// zero == menuba
  int numitems
  int selected
  int drawn_selected;	// last redraw has this selecte
  const Fl_Menu_Item* menu
...
#define LEADING 4 // extra vertical leadin
...
void Fl_Menu_Item::draw(int x, int y, int w, int h, const Fl_Menu_* m
  ...
  l.font    = labelsize_ || labelfont_ ? labelfont_ : uchar(m ? m->textfont() : FL_HELVETICA)
  l.size    = labelsize_ ? labelsize_ : m ? m->textsize() : (uchar)FL_NORMAL_SIZE
  l.color   = labelcolor_ ? labelcolor_ : m ? m->textcolor() : int(FL_FOREGROUND_COLOR); //was:FL_BLAC
  ...
  if (selected) 
    ...
    if (fl_contrast(r,color)!=r) { // back compatability boxtype
      if (selected == 2) { // menu titl
	r = color
	b = m ? m->box() : FL_UP_BOX
      } else 
	r = (Fl_Color)(FL_COLOR_CUBE-1); // whit
...
menuwindow::menuwindow(const Fl_Menu_Item* m, int X, int Y, int Wp, int Hp
  ...
  if ( gd ) 
    // since the menu pops over everything, we use the scree
    // bounds, right across the dock and menu ba
    GDPtr gp = *gd
  ...
  clear_border()
  menu = m
  if (m) m = m->first(); // find the first item that needs to be rendere
  drawn_selected = -1
  if (button) 
    box(button->box())
...
void menuwindow::position(int X, int Y) 
  ...
  // x(X); y(Y); // don't wait for response from 


// scroll so item i is visible on scree
void menuwindow::autoscroll(int n) 
  ...
  // y(y()+Y); // don't wait for response from 


///////////////////////////////////////////////////////////////

void menuwindow::drawentry(const Fl_Menu_Item* m, int n, int eraseit) 
  if (!m) return; // this happens if -1 is selected item and redraw
...
void menuwindow::draw() 
  if (damage() != FL_DAMAGE_CHILD) {	// complete redra
    ...
    if (damage() & FL_DAMAGE_CHILD && selected!=drawn_selected) { // change selectio
...
// But the handle method needs to look at all of them to find ou
// what item the user is pointing at.  And it needs a whole lo
// of other state variables to determine what is going on wit
// the currently displayed menus
// So the main loop (handlemenu()) puts all the state in a structur
// and puts a pointer to it in a static location, so the handle(
// on menus can refer to it and alter it.  The handle() metho
// changes variables in this state to indicate what item i
// picked, but does not actually alter the display, instead th
// main loop does that.  This is because the X mapping and unmappin
// of windows is slow, and we don't want to fall behind the events

// values for menustate.state
#define INITIAL_STATE 0	// no mouse up or down since popup() calle
#define PUSH_STATE 1	// mouse has been pushed on a normal ite
#define DONE_STATE 2	// exit the popup, the current item was picke
#define MENU_PUSH_STATE 3 // mouse has been pushed on a menu titl

struct menustate 
  const Fl_Menu_Item* current_item; // what mouse is pointing a
  int menu_number; // which menu it is i
  int item_number; // which item in that menu, -1 if non
  menuwindow* p[20]; // pointers to menu
  int nummenus
  int menubar; // if true p[0] is a menuba
  int state
}
static menustate* p
...
static int forward(int menu) { // go to next item in menu menu if possibl
  menustate &pp = *p
...
static int backward(int menu) { // previous item in menu menu if possibl
  menustate &pp = *p
  menuwindow &m = *(pp.p[menu])
  int item = (menu == pp.menu_number) ? pp.item_number : m.selected
...
int menuwindow::handle(int e) 
  ...
  case FL_DRAG: 
    ...
    if (e == FL_PUSH) 
      if (pp.current_item && pp.current_item->submenu() // this is a menu titl
	  && item != pp.p[mymenu]->selected // and it is not already o
	  && !pp.current_item->callback_) // and it does not have a callbac
	pp.state = MENU_PUSH_STATE
      els
	pp.state = PUSH_STATE
    }} return 1
  case FL_RELEASE
    // do nothing if they try to pick inactive item
    if (pp.current_item && !pp.current_item->activevisible()) return 1
    // Mouse must either be held down/dragged some, or this must b
    // the second click (not the one that popped up the menu)
    if (!Fl::event_is_click() || pp.state == PUSH_STATE |
	pp.menubar && pp.current_item && !pp.current_item->submenu() // butto
	) 
#if 0 // makes the check/radio items leave the menu u
...
const Fl_Menu_Item* Fl_Menu_Item::pulldown
  ...
  menuwindow* fakemenu = 0; // kludge for buttons in menuba
  ...
  if (menubar) mw.handle(FL_DRAG); // find the initial men
  ...
  for (;;) 
    ...
    if (!pp.current_item) { // pointing at nothin
    ...
    initial_item = 0; // stop the startup cod
    pp.p[pp.menu_number]->autoscroll(pp.item_number)

  STARTUP
    menuwindow& cw = *pp.p[pp.menu_number]
    const Fl_Menu_Item* m = pp.current_item
    if (!m->activevisible()) { // pointing at inactive ite
      cw.set_selected(-1)
      initial_item = 0; // turn off startup cod
      continue
    
    cw.set_selected(pp.item_number)

    if (m==initial_item) initial_item=0; // stop the startup code if item foun
    if (m->submenu()) 
...
Fl_Menu_Item::popup
  ...
  static Fl_Menu_Item dummy; // static so it is all zero
  dummy.text = title
  return pulldown(X, Y, 0, 0, picked, but, title ? &dummy : 0)


// Search only the top level menu for a shortcut.  Either &x in th
...
// Recursive search of all submenus for anything with this key as 
