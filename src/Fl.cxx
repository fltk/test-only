...
/
// 'Fl:event_inside()' - Return whether or not the mouse event is insid
//                       the given rectangle
/
...
///////////////////////////////////////////////////////////////
// Timeouts are stored in a sorted list, so only the first one need
...
// I avoid the overhead of getting the current time when we have n
// timeouts by setting this flag instead of getting the time
// In this case calling elapse_timeouts() does nothing, but record
...
// Continuously-adjusted error value, this is a number <= 0 for how lat
// we were at calling the last timeout. This appears to make repeat_timeou
// very accurate even when processing takes a significant portion of th
// time interval
...
///////////////////////////////////////////////////////////////
// Checks are just stored in a list. They are called in the revers
// order that they were added (this may change in the future)
// This is a bit messy because I want to allow checks to be added
// removed, and have wait() called from inside them, to do thi
// next_check points at the next unprocessed one for the outermos
// call to Fl::wait()
...
void (*Fl::idle)(); // see Fl_add_idle.cxx for the add/remove function

extern int fl_ready(); // in Fl_<platform>.cx
extern int fl_wait(double time); // in Fl_<platform>.cx
...
double Fl::wait(double time_to_wait) 
  ...
  if (first_timeout) 
    ...
    reset_clock = 1; // we are not going to check the cloc
  
  // checks are a bit messy so that add/remove and wait may be calle
...
int Fl::wait() 
  wait(FOREVER)
  return Fl_X::first != 0; // return true if there is a windo
...
Fl_Window* fl_find(Window xid) 
  ...
  for (Fl_X **pp = &Fl_X::first; (window = *pp); pp = &window->next
	...
	// make this window be first to speed up searche
	// this is not done if modal is true to avoid messing up modal stac
	*pp = window->next
...
int (*fl_local_grab)(int); // used by fl_dnd.cx
...
void Fl::focus(Fl_Widget *o) 
  ...
  if (grab()) return; // don't do anything while grab is o
  Fl_Widget *p = focus_
  if (o != p) 
    Fl::compose_reset()
    focus_ = o
...
static char dnd_flag = 0; // make 'belowmouse' send DND_LEAVE instead of LEAV
...
Fl_Window *fl_xfocus;	// which window X thinks has focu
Fl_Window *fl_xmousewin;// which window X thinks has FL_ENTE
Fl_Window *Fl::grab_;	// most recent Fl::grab(
Fl_Window *Fl::modal_;	// topmost modal() windo
...
// This is the only function that produces these events in respons
// to system activity
// This is called whenever a window is added or hidden, and wheneve
...
// MRS: Originally we checked the button state, but a user reported that i
...
//  if (!(Fl::event_state() & 0x7f00000 /*FL_BUTTONS*/)) 
  if (!Fl::pushed()) 
	...
	// send a FL_MOVE event so the enter/leave state is up to dat
...
#ifndef WIN3
extern Fl_Widget *fl_selection_requestor; // from Fl_x.cx
#endi

// This function is called by ~Fl_Widget() and by Fl_Widget::deactivat
// and by Fl_Widget::hide().  It indicates that the widget does not wan
// to receive any more events, and also removes all global variables tha
// point at the widget
// I changed this from the 1.0.1 behavior, the older version could sen
// FL_LEAVE or FL_UNFOCUS events to the widget.  This appears to not b
...
// Call to->handle but first replace the mouse x/y with the correc
// values to account for nested X windows. 'window' is the outermos
...
//           window, pushed(), grab(), modal())
  ...
  case FL_KEYBOARD
    ...
    // and then try a shortcut with the case of the text swapped, b
    // changing the text and falling through to FL_SHORTCUT case
    {char* c = (char*)event_text(); // cast away cons
    if (!isalpha(*c)) return 0
    *c = isupper(*c) ? tolower(*c) : toupper(*c);
    e_number = e = FL_SHORTCUT

  case FL_SHORTCUT
    if (grab()) {wi = grab(); break;} // send it to grab windo
...
#ifdef WIN3
  // Try to stop the annoying "raise another program" behavio
...
// FL_SHOW and FL_HIDE are called whenever the visibility of this widge
// or any parent changes.  We must correctly map/unmap the system's window

// For top-level windows it is assummed the window has already bee
// mapped or unmapped!!!  This is because this should only happen whe
// Fl_Window::show() or Fl_Window::hide() is called, or in response t
...
int Fl_Window::handle(int ev
  ...
  if (parent()) 
    ...
    case FL_SHOW
      if (!shown()) show()
      else XMapWindow(fl_display, fl_xid(this)); // extra map calls are harmles
      break
    case FL_HIDE
      if (shown()) 
	// Find what really turned invisible, if is was a parent windo
	// we do nothing.  We need to avoid unnecessary unmap call
	// because they cause the display to blink when the parent i
	// remapped.  However if this or any intermediate non-windo
	// widget has really had hide() called directly on it, we mus
	// unmap because when the parent window is remapped we don'
	// want to reappear
	if (visible()) 
	 Fl_Widget* p = parent(); for (;p->visible();p = p->parent()) {
	 if (p->type() >= FL_WINDOW) break; // don't do the unma
...
void Fl_Widget::redraw_label() 
  if (window()) 
    if (box() == FL_NO_BOX) 
      // Widgets with the FL_NO_BOX boxtype need a parent t
      // redraw, since it is responsible for redrawing th
      // background..
      int X = x() > 0 ? x() - 1 : 0
      int Y = y() > 0 ? y() - 1 : 0
    ...
    if (align() && !(align() & FL_ALIGN_INSIDE) && window()->shown()) 
      // If the label is not inside the widget, compute the location o
      // the label and redraw the window within that bounding box..
      int W = 0, H = 0
      measure_label(W, H)
      W += 5; // Add a little to the size of the label to cover overflo
      H += 5
...
void Fl_Widget::damage(uchar fl) 
  if (type() < FL_WINDOW) 
    ...
    if (!i) return; // window not mapped, so ignore i
    if (i->region) {XDestroyRegion(i->region); i->region = 0;
    damage_ |= fl
...
/
// The following methods allow callbacks to schedule the deletion o
