...
void Fl_Roller::draw() 
  ...
  const double ARC = 1.5; // 1/2 the number of radians visibl
  const double delta = .2; // radians per knur
  if (horizontal()) { // horizontal on
    // draw shaded ends of wheel
    int h1 = W/4+1; // distance from end that shading start
	...
	fl_color(FL_LIGHT1);fl_yxline(X+yy1,Y+1,Y+H-1)
      ...
      // draw edges
      h1 = W/8+1; // distance from end the color invert
      fl_color(FL_DARK2)
    ...
    // draw shaded ends of wheel
    int h1 = H/4+1; // distance from end that shading start
	...
	fl_color(FL_LIGHT1);fl_xyline(X+1,Y+yy1,X+W-1)
      ...
      // draw edges
      h1 = H/8+1; // distance from end the color invert
      fl_color(FL_DARK2)
