...
// These are small graphics drawn by the normal label-drawin
// code when the string starts with an '@' sign

// Adapted from original code written by

// Written by Mark Overmar
// Version 2.1 
// Date: Oct  2, 199
...
#define MAXSYMBOL       21
   /* Maximal number of symbols in table. Only half of them ar
...
/* Adds a symbol to the system. Returns whether correct. *
  ...
  if(! symbols[pos].notempty)
    if (symbnumb > MAXSYMBOL / 2) return -1;	// table is ful
    symbnumb++
...
int fl_draw_symbol(const char *label,int x,int y,int w,int h,Fl_Color col) { 
  ...
  if (symbols[pos].scalable == 3) { // kludge to detect return arro
    fl_color(col)
...
Fl_Symbol & Fl_Symbol::set(void (* draw)(Fl_Color c), uchar rot){
  ...
  // draw functions for these constructors draw dirrectly within x,y,w,h area. If square is se
  // the dwawimg is resized so that w==h whichever is smaller.and centered in the are
  // 4 least significant bits of type_ must be unique for particular function. Type nost be even ( |1) it it is scalabl
