...
// Methods on Fl_Gl_Window that create an overlay window.  Becaus
// many programs don't need the overlay, this is seperated into thi
// source file so it is not linked in if not used

// Under X this is done by creating another window, of class _Fl_Gl_Overla
...
// Under win32 another GLX context is created to draw into the overla
// and it is stored in into the "overlay" pointer

// In both cases if overlay hardware is unavailable, the overlay i
// "faked" by drawing into the main layers.  This is indicated b
...
void _Fl_Gl_Overlay::flush() 
  make_current()
#ifdef BOXX_BUG
  // The BoXX overlay is broken and you must not call swap-buffers. Thi
  // code will make it work, but we lose because machines that do suppor
  // double-buffered overlays will blink when they don't have t
...
void Fl_Gl_Window::make_overlay() 
  ...
  if (!context) {overlay = this; return;} // fake the overla
