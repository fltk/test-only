...
// Platform specific part is in sub-directorie
...
//      If you want to test/fix this, change #ifdef 1 to 0 here  and change #ifdef 0 to 
//      for line with //COPYDEEPMASK in mac/quickdraw subdirectory.  Also see Fl_Image.cxx for a similar check..


#if 1 && !defined(__APPLE__

// Create a 1-bit mask used for alpha blendin
Fl_Bitmask fl_create_alphamask(int w, int h, int d, int ld, const uchar *array) 
  ...
  const uchar *dataptr
  int x, y
  static uchar dither[16][16] = { // Simple 16x16 Floyd dithe
  ...
  // Generate a 1-bit "screen door" alpha mask; not always pretty, bu
  // definitely fast...  In the future we may be able to support thing
  // like the RENDER extension in XFree86, when available, to provid
  // true RGBA-blended rendering.  See
  /
  //     http://www.xfree86.org/~keithp/render/protocol.htm
  /
  // for more info on XRender..
  /
  // MacOS already provides alpha blending support and has its ow
...
void Fl_Bitmap::draw(int XP, int YP, int WP, int HP, int cx, int cy) 
  ...
  // RK drawing to the devic
  fl_device->draw(this, X, Y, W, H, cx, cy)


Fl_Bitmap::~Fl_Bitmap() 
  //uncache(); //RK: in base image clas
...
Fl_Image *Fl_Bitmap::copy(int W, int H) 
  Fl_Bitmap     *new_image;     // New RGB imag
  ...
  // OK, need to resize the image data; allocate memory and
    uchar         *new_ptr,       // Pointer into new arra
		new_bit,	// Bit for new arra
		old_bit;	// Bit for old arra
  const uchar	*old_ptr;	// Pointer into old arra
  int		sx, sy,		// Source coordinate
