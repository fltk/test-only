...
void Fl_Scroll::draw_clip(void* v,int X, int Y, int W, int H) 
  ...
  Fl_Boxtype d_box = s->box();
  if(d_box == FL_NO_BOX |
    d_box == FL_UP_FRAME |
    d_box == FL_DOWN_FRAME |
    d_box == FL_THIN_UP_FRAME |
    d_box ==  FL_THIN_DOWN_FRAME |
    d_box == FL_ENGRAVED_FRAME |
    d_box == FL_EMBOSSED_FRAME |
    d_box == FL_BORDER_FRAME |
    d_box == _FL_SHADOW_FRAME |
    d_box == _FL_ROUNDED_FRAME |
    d_box == _FL_OVAL_FRAME  |
    d_box == _FL_PLASTIC_UP_FRAME |
    d_box == _FL_PLASTIC_DOWN_FRAME )
...
void Fl_Scroll::draw() 
  ...
  if (d & FL_DAMAGE_ALL) { // full redra
    ...
    if (d & FL_DAMAGE_CHILD) { // draw damaged childre
      fl_clip(X, Y, W, H)
      Fl_Widget*const* a = array()
  ...
  if (type() & HORIZONTAL) 
    if ((type() & ALWAYS_ON) || l < X || r > X+W) 
      ...
      H -= hscrollbar.h()
      if (scrollbar.align() & FL_ALIGN_TOP) Y += hscrollbar.h()
      // recheck vertical since we added a horizontal scrollba
  ...
  if (d & FL_DAMAGE_ALL) 
    ...
    if (scrollbar.visible() && hscrollbar.visible()) 
      // fill in the little box in the corne
      fl_color(color())
