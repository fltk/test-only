...
// These are for Forms emulation and for dynamically changing th
// menus.  They are in this source file so they are not linked in i
// not used, which is what will happen if the the program only use
// constant menu tables

// Not at all guaranteed to be Forms compatable, especially with an
...
static int local_array_alloc = 0; // number allocate
static int local_array_size = 0; // == size(local_array
extern Fl_Menu_* fl_menu_array_owner; // in Fl_Menu_.cx

// For historical reasons there are matching methods that work on 
// user-allocated array of Fl_Menu_Item.  These methods are quit
// depreciated and should not be used.  These old methods use th
// above pointers to detect if the array belongs to an Fl_Menu
...
// Add an item.  The text is split at '/' characters to automaticall
// produce submenus (actually a totally unnecessary feature as you ca
...
int Fl_Menu_Item::add
  ...
  if (!m->text) {	/* add a new menu item *
    ...
    msize++
    if (myflags & FL_SUBMENU) { // add submenu delimite
      array = insert(array, msize, n+1, 0, 0)
      msize++
...
int Fl_Menu_::add(const char *t, int s, Fl_Callback *c,void *v,int f) 
  ...
  if (this != fl_menu_array_owner) 
    ...
    if (menu_) 
      ...
      delete[] local_array
      if (!alloc) copy(menu_); // duplicate a user-provided static arra
      // add to the menu's current array
      local_array_alloc = local_array_size = size()
      local_array = menu_
    } else 
      // start with a blank array
      alloc = 2; // indicates that the strings can be free
      if (local_array) 
	menu_ = local_array
      } else 
  ...
  menu_ = local_array; // in case it reallocated i
  if (value_) value_ = menu_+value_offset
  return r


// This is a Forms (and SGI GL library) compatable add function, i
// adds many menu items, with '|' seperating the menu items, and ta
