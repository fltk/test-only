...
// On systems that support double buffering "naturally" the bas
// Fl_Window class will probably do double-buffer and this subclas
// does nothing
...
static int can_xdbe() 
  ...
  if (!tried) 
    ...
    for (int j = 0; j < a->count; j++
      if (a->visinfo[j].visual == fl_visual->visuali
...
void Fl_Double_Window::show() 
  ...
  if (!shown()) { // don't set the background pixe
    fl_open_display()
    Fl_X::make_xid(this)
    return
...
// Code used to switch output to an off-screen window.  See macros i
...
void fl_begin_offscreen(GWorldPtr gWorld) 
  ...
  if ( gWorld 
    ...
    SetGWorld( gWorld, 0 ); // sets the correct por
...
// Fl_Overlay_Window relies on flush(1) copying the back buffer to th
...
void Fl_Double_Window::flush(int eraseoverlay) 
  make_current(); // make sure fl_gc is non-zer
...
#endi
#ifdef __APPLE_
    // the Apple OS X window manager double buffers ALL window
    // anyway, so there is no need to waste memory and time
    /
    // BTW: Windows2000 and later also forces doublebuffering i
  ...
  if (damage() & ~FL_DAMAGE_EXPOSE) 
    ...
    fl_restore_clip(); // duplicate region into new g
    draw()
    DeleteDC(fl_gc)
    fl_gc = _sgc
#elif defined(__APPLE__
  ...
  // on Irix (at least) it is faster to reduce the area copied t
