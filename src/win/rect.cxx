...
void Fl_Win_Display::line(int x, int y, int x1, int y1) 
  ...
  MoveToEx(fl_gc, x, y, 0L);
  LineTo(fl_gc, x1, y1)
  // Draw the last point *again* because the GDI line drawin
  // functions will not draw the last point ("it's a feature!"...
  SetPixel(fl_gc, x1, y1, fl_RGB())
  SetPixel(fl_gc, x, y, fl_RGB()); // has to do als
...
void fl_restore_clip() 
  ...
  SelectClipRgn(fl_gc, r); //if r is NULL, clip is automatically cleare
...
// The win32 API makes no distinction between partial and complet
// intersection, so we have to check for partial intersection ourselves
// However, given that the regions may be composite, we have to d
// some voodoo stuff..
  Fl_Region rr = XRectangleRegion(x,y,w,h)
  Fl_Region temp = CreateRectRgn(0,0,0,0)
  int ret
  if (CombineRgn(temp, rr, r, RGN_AND) == NULLREGION) { // disjoin
    W = H = 0
    ret = 2
  } else if (EqualRgn(temp, rr)) { // complet
    ret = 0
  } else {	// parital intersectio
    RECT rect
    GetRgnBox(temp, &rect)
