...
// Stock pen is used for 1 (0) width  solid line
// Other lines are use pens from pen_cach
...
#define PEN_CACHE_SIZE  16  // size of cache for non-stock pen
...
FL_EXPORT HPEN fl_pen()
  ...
  Pen_Cache * pen = first_pen
	...
	// pen not found, need to create on
	if(no_pens == PEN_CACHE_SIZE){ // cache is full, need to delete the one used the longest time ago. We know that the pen is not selected
		...
		prev = pen_cache + no_pens++; // where to place new pe
...
HBRUSH fl_brush() 
  ...
  if(current_brush && (for_color == current_brush_color))
	  SelectObject(fl_gc, current_brush); //assure that we have selected the brush for proper gc if it changed by any chanc
...
#define FL_N_PEN 32 // number of non-stock pen
...
static void clear_xmap(Fl_XMap& xmap) 
  if (xmap.pen) 
    ...
    HPEN oldpen = (HPEN)SelectObject(fl_gc, tmppen); // Push out the current pen of the g
    if(oldpen != xmap.pen) SelectObject(fl_gc, oldpen); // Put it back if it is not the one we are about to delet
    SelectObject(fl_gc, tmpbrush); // Push out the old pen of the g
    //fl_current_xmap = 0
...
// Wonko: we use some statistics to cache only a limited numbe
// of brushes
...
HBRUSH fl_brush() 
  ...
  int i = xmap->brush; // find the associated brus
  if (i != -1) { // if the brush was allready allocate
    if (brushes[i].brush == NULL) goto CREATE_BRUSH
    if ( (++brushes[i].usage) > 32000 ) { // keep a usage statisti
...
// 'fl_select_palette()' - Make a color palette for 8-bit displays if necessar
// Thanks to Michael Sweet @ Easy Software Products for thi
...
fl_select_palette(void
  ...
  if (!beenhere) 
    ...
    // this will try to work on < 256 color screens, but will probabl
...
FL_EXPORT void push_xmaps()
  if(fl_gc_save) return; //allready pushe
