...
// USA
/
// Please report all bugs and problems to "fltk-bugs@fltk.or
...
/*
 * subclass the IDropTarget to receive data from DnD operation
 *
class FLDropTarget : public IDropTarge

  DWORD m_cRefCount
  DWORD lastEffect
  int px, py
public
  FLDropTarget() : m_cRefCount(0) { } // initializ
  ...
  HRESULT STDMETHODCALLTYPE DragEnter( IDataObject *pDataObj, DWORD /*grfKeyState*/, POINTL pt, DWORD *pdwEffect) 
    ...
    // set e_modifiers here from grfKeyState, set e_x and e_root_
    ...
    px = pt.x; py = pt.y
      // FLTK has no mechanism yet for the different drop effects, so we allow move and cop
  ...
  HRESULT STDMETHODCALLTYPE DragOver( DWORD /*grfKeyState*/, POINTL pt, DWORD *pdwEffect) 
    ...
    // Fl_Group will change DND_DRAG into DND_ENTER and DND_LEAVE if neede
  ...
  HRESULT STDMETHODCALLTYPE Drop( IDataObject *data, DWORD /*grfKeyState*/, POINTL pt, DWORD* /*pdwEffect*/) 
    ...
    // tell FLTK that the user released an object on this widge
    ...
    // if it is ASCII text, send an FL_PASTE with that tex
    if ( data->GetData( &fmt, &medium )==S_OK 
      ...
      Fl::belowmouse()->handle(Fl::e_number = FL_PASTE); // e_text will be invalid after this cal
...
/*
 * this class is needed to allow FLTK apps to be a DnD sourc
 *
class FLDropSource : public IDropSourc
...
/*
 * this is the actual object that FLTK can drop somewher
 * - the implementation is minimal, but it should work with all decent Win32 drop target
 *
class FLDataObject : public IDataObjec
...
public
  ...
  // GetData currently allows ASCII text through Global Memory onl
  ...
  // all the following methods are not really needed for a DnD objec
...
/*
 * drag and drop whatever is in the cut-copy-paste buffe
...
int Fl::dnd(
  ...
  if ( ret==DRAGDROP_S_DROP ) return 1; // or DD_S_CANCE
  return 0
