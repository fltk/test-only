...
static Fl_Bitmask fl_create_bitmap(int w, int h, const uchar *data) 
  // we need to pad the lines out to words & swap the bit
  // in each byte
  int w1 = (w+7)/8
  int w2 = ((w+15)/16)*2
  ...
  uchar* dest = newarray
  Fl_Bitmask id
  static uchar reverse[16] =	// Bit reversal lookup tabl
...
Fl_Bitmask fl_create_bitmask(int w, int h, const uchar *data) 
  // this won't work when the user changes display mode during run o
  // has two screens with differnet depth
  Fl_Bitmask id
  ...
  int np  = GetDeviceCaps(fl_gc, PLANES);	//: was always one on sample machine
  int bpp = GetDeviceCaps(fl_gc, BITSPIXEL);//: 1,4,8,16,24,32 and more odd stuff
  int Bpr = (bpp*w+7)/8;			//: bytes per ro
  ...
  for (int i=0; i<h; i++) 
    // This is slooow, but we do it only once per pixma
    for (int j=w1; j>0; j--) 
      uchar b = *src++
      if (bpp==1) 
