...
// This file contains win32-specific code for fltk which is always linke
// in.  Search other files for "WIN32" or filenames ending in _win32.cx
...
//#define USE_ASYNC_SELEC


/
// USE_TRACK_MOUSE - define it if you have TrackMouseEvent()..
/
// Apparently, at least some versions of Cygwin/MingW don't provid
// the TrackMouseEvent() function.  You can define this by han
// if you have it - this is only needed to support subwindo
// enter/leave notification under Windows
/

//#define USE_TRACK_MOUS
...
/
// WM_SYNCPAINT is an "undocumented" message, which is finally defined i
// VC++ 6.0
/

#ifndef WM_SYNCPAIN
#  define WM_SYNCPAINT 0x008
#endi

#ifndef WM_MOUSELEAV
#  define WM_MOUSELEAVE 0x02a
#endi

#ifndef WM_MOUSEWHEE
#  define WM_MOUSEWHEEL 0x020
#endi
...
/
// WM_FLSELECT is the user-defined message that we get when one o
...
// fd's are only implemented for sockets.  Microsoft Windows does no
// have a unified IO system, so it doesn't support select() on files
// devices, or pipes..
/
// Microsoft provides the Berkeley select() call and an asynchronou
// select function that sends a WIN32 message when the select conditio
// exists..
static int maxfd = 0
#ifndef USE_ASYNC_SELEC
static fd_set fdsets[3]
#endif // !USE_ASYNC_SELEC
...
// timeout, and >0 if any callbacks were done.  This version onl
// returns zero if nothing happens during a 0.0 timeout, otherwis
// it returns 1
int fl_wait(double time_to_wait) 
...
#ifndef USE_ASYNC_SELEC
  if (nfds) 
    // For WIN32 we need to poll for socket input FIRST, sinc
	...
	if (fd[i].events & revents) fd[i].cb(f, fd[i].arg)
      ...
      time_to_wait = 0.0; // just peek for any message
#ifdef __CYGWIN_
  ...
  while (have_message) 
    ...
    if (fl_msg.message == fl_wake_msg)  // Used for awaking wait() from another threa
...
static int mouse_event(Fl_Window *window, int what, int button
  ...
  ulong state = Fl::e_state & 0xff0000; // keep shift key state
#if 
...
// See also the inverse converter in Fl_get_key_win32.cx
...
static const struct {unsigned short vk, fltk, extended;} vktab[] = 
  ...
  {VK_SNAPSHOT,	FL_Print},	// does not work on N
...
#if USE_COLORMA
extern HPALETTE fl_select_palette(void); // in fl_color_win32.cx
#endi
...
static LRESULT CALLBACK WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam
  ...
  // Copy the message to fl_msg so add_handler code can see it, it i
  // already there if this is called by DispatchMessage, but not i
  ...
  case WM_CLOSE: // user clicked close bo
  ...
  case WM_NCPAINT 
  case WM_ERASEBKGND 
    // Andreas Weitl - WM_SYNCPAINT needs to be passed to DefWindowPro
  ...
  case WM_PAINT: 
    ...
    // These next two statements should not be here, so that all updat
    // is deferred until Fl::flush() is called during idle.  However WIN3
  ...
  case WM_ACTIVATEAPP
    // From eric@vfx.sel.sony.com, we should process WM_ACTIVATEAP
    // messages to restore the correct state of the shift/ctrl/alt/loc
    // keys...  Added control, shift, alt, and meta keys, and change
    // to use GetAsyncKeyState and do it when wParam is 
  ...
  case WM_CHAR
  case WM_SYSCHAR: 
    ulong state = Fl::e_state & 0xff000000; // keep the mouse button stat
    ...
    if ((GetKeyState(VK_LWIN)|GetKeyState(VK_RWIN))&~1) 
      // WIN32 bug?  GetKeyState returns garbage if the user hit th
  ...
  case WM_MOUSEWHEEL: 
    static int delta = 0; // running total of all motio
  ...
  case WM_RENDERALLFORMATS
    fl_i_own_selection[1] = 0
    // Windoze seems unhappy unless I do these two steps. Documentatio
    // seems to vary on whether opening the clipboard is necessary o
    // is in fact wrong
    CloseClipboard()
    OpenClipboard(NULL)
    // fall through..
  case WM_RENDERFORMAT: 
    ...
    // Windoze also seems unhappy if I don't do this. Documentation ver
...
///////////////////////////////////////////////////////////////
// This function gets the dimensions of the top/left borders an
// the title bar, if there is one, based on the FL_BORDER, FL_MODA
// and FL_NONMODAL flags, and on the window's size range
// It returns the following values
/
// value | border | title ba
//   0   |  none  |   n
//   1   |  fix   |   ye
//   2   |  size  |   ye
...
int Fl_X::fake_X_wm(const Fl_Window* w,int &X,int &Y, int &bt,int &bx, int &by) 
  ...
  //Make border's lower right corner visibl
  if (Fl::w() < X+W) X = Fl::w() - W
  if (Fl::h() < Y+H) Y = Fl::h() - H
  //Make border's upper left corner visibl
  if (X<0) X = 0
  if (Y<0) Y = 0
  //Make client area's lower right corner visibl
  if (Fl::w() < X+dx+ w->w()) X = Fl::w() - w->w() - dx
  if (Fl::h() < Y+dy+ w->h()) Y = Fl::h() - w->h() - dy
  //Make client area's upper left corner visibl
  if (X+xoff < 0) X = -xoff
  if (Y+yoff < 0) Y = -yoff
...
void Fl_Window::resize(int X,int Y,int W,int H) 
  ...
  if (resize_from_program && shown()) 
    ...
    int dummy, bt, bx, by
    //Ignore window managing when resizing, so that windows (and mor
...
// int fl_background_pixel = -1; // color to use for backgroun
...
Fl_X* Fl_X::make(Fl_Window* w) 
  ...
  if (!class_name) class_name =*/ "FLTK"; // create a "FLTK" WNDCLAS

  const char* message_name = "FLTK::ThreadWakeup"

  WNDCLASSEX wc
  // Documentation states a device context consumes about 800 byte
  // of memory... so who cares? If 800 bytes per window is what i
  ...
  if (w->parent()) 
    ...
    switch (fake_X_wm(w, xwm, ywm, bt, bx, by)) 
      ...
      // Thin border and title ba
      case 1: style |= WS_DLGFRAME | WS_CAPTION; break

      // Thick, resizable border and title bar, with maximize butto
  ...
  if (showit) 
    ...
    w->handle(Fl::e_number = FL_SHOW); // get child windows to appea
    Fl::e_number = old_event
    w->redraw(); // force draw to happe
  
  // If we've captured the mouse, we dont want do activate an
...
#if !defined(__GNUC__) || __GNUC__ >= 
  // Register all windows for potential drag'n'drop operation
...
#include <FL/filename.H> // need so FL_EXPORT fl_filename_name work
...
const char *fl_filename_name(const char *name) 
  ...
  if (!name) return (0)
  q = name
  if (q[0] && q[1]==':') q += 2; // skip leading drive lette
...
// For WIN32 it looks like all windows share a background color, s
// I use FL_GRAY for this and only do this cheat for windows that ar
// that color
...
// the current contex
HDC fl_gc = 0
// the current window handle, initially set to -1 so we can correctl
// allocate fl_GetDC(0
...
#if USE_COLORMA
  // Windows maintains a hardware and software color palette; th
  // SelectPalette() call updates the current soft->hard mappin
  // for all drawing calls, so we must select it here before an
