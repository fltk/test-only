...
unsigned	Fl_Tooltip::color_ = fl_color_cube(FL_NUM_RED - 1
		                                   FL_NUM_GREEN - 1
						   FL_NUM_BLUE - 2)
unsigned	Fl_Tooltip::textcolor_ = FL_FOREGROUND_COLOR; //was:FL_BLAC
...
// If this widget or one of it's parents has a tooltip, enter it. Thi
// will do nothing if this is the current widget (even if the mouse move
// out so an exit() was done and then moved back in). If no tooltip ca
// be found do Fl_Tooltip::exit_(). If you don't want this behavior (for instanc
...
// Acts as though enter(widget) was done but does not pop up 
// tooltip.  This is useful to prevent a tooltip from reappearing whe
// a modal overlapping window is deleted. FLTK does this automaticall
...
// Get ready to display a tooltip. The widget and the xywh box insid
// it define an area the tooltip is for, this along with the curren
// mouse position places the tooltip (the mouse is assummed to poin
...
#ifdef WIN3
    // possible fix for the Windows titlebar, it seems to want th
