 ...
 * It has been tested on the "menubar" sample program and provide
 * basic functionality.
 *
 * To use the System Menu Bar, simply replace the main Fl_Menu_Ba
 * in an application with Fl_Sys_Menu_Bar
 
 * FLTK features not supported by the Mac System men
 
 * - no invisible menu item
 * - no sybolic label
 * - embossed labels will be underlined instea
 * - no font size
 * - Shortcut Characters should be English alphanumeric only, no modifiers ye
 * - no disable main menu
 ...
 * Skip all '&' which would mark the shortcut in FLT
...
/*
 * append a marker to identify the menu font styl
 * <B, I, U, O, and 
 *
...
/*
 * append a marker to identify the menu shortcu
 * <B, I, U, O, and 
enum 
  kMenuNoModifiers = 0
...
/*
 * create a sub menu for a specific menu handl
 *
...
/*
 * create a system menu bar using the given list of menu struct
 
 * \author Matthias Melche
 
 * @param m list of Fl_Menu_Ite
 *
void Fl_Sys_Menu_Bar::menu(const Fl_Menu_Item *m)
  ...
  char buf[255]
  int cnt = 1; // first menu is no 2. no 1 is the Apple Men
...
/* -- nothing to do, system should take care of thi
  draw_box()
