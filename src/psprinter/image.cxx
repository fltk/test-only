...
//  uchar d
  ...
  if(!V_)
    if(V0
      if(V255){// not true alpha, only maskin
        xx = (w+7)/8
        mask = new uchar[h * xx]
  ...
  for(i=0;i<((w+1)/2) * h * 4; i++) mask[i] = 0; //cleanin



  mx= w*4
  my=h*4; // mask dimension

  xx = (w+1)/2;                //  mask line width in byte

  short * errors1 = new short [w*4+2]; //  two rows of dither error
  short * errors2 = new short [w*4+2]; //  two rows of dither error

  for(i=0;i<w*4+2;i++) errors2[i] = 0; // cleaning,after first swap will become curren
  for(i=0;i<w*4+2;i++) errors1[i] = 0; // cleaning,after first swap will become curren
  ...
  for(j=0;j<h;j++)
    for(l=0;l<4;){           // generating 4 rows of mask lines for 1 RGB lin
      int jj = j*4+l
      ...
      next = current
      current = swap
      *(next+1) = 0;          // must clean the first cell, next are overriden by *
      for(i=0;i<w;i++)
        for(k=0;k<4;k++){   // generating 4 x-pixels for 1 RG
          short error, o1, o2, o3
          int ii = i*4+k;   // mask cell inde
          short val = data[j*LD+D*i+D-1] + current[1+ii]
          if (val>127)
            mask[jj*xx+ii/8]  |= 1 << (ii % 8); //set mask bi
            error =  val-255
          }els
            error = val
      ...
      *(next+1) = 0;          // must clean the first cell, next are overriden by *
      ...
      for(i=w-1;i>=0;i--)
        ...
        for(k=3;k>=0;k--){   // generating 4 x-pixels for 1 RG
          short error, o1, o2, o3
  ...
  for (j=0; j<ih;j++)
    ...
    for(i=0 ; i<iw ; i++) 
      ...
      if(lang_level_<3 && D>3) { //can do  mixing using bg_* colors
        unsigned int a2 = curdata[3]; //must be in
...
void Fl_PS_Printer::draw_scalled_image(Fl_Draw_Image_Cb call, void *data, double x, double y, double w, double h, int iw, int ih, int D) 
  ...
  for (j=0; j<ih;j++)
    if(mask && lang_level_>2){  // InterleaveType 2 mask dat
      for(k=0; k<my/ih;k++){ //for alpha pseudo-maskin
        for (i=0; i<((mx+7)/8);i++)
          if (!(i%40)) fprintf(output, "\n")
          fprintf(output, "%.2x",swap_byte(*curmask))
          curmask++
...
void Fl_PS_Printer::draw_scalled_image_mono(const uchar *data, double x, double y, double w, double h, int iw, int ih, int D, int LD) 
  ...
  for (j=0; j<ih;j++)
    ...
    for(i=0 ; i<iw ; i++) 
      if (!(i%80)) fprintf(output, "\n")
      uchar r = curdata[0]
      if(lang_level_<3 && D>1) { //can do  mixin

        unsigned int a2 = curdata[1]; //must be in
        unsigned int a = 255-a2
        r = (a2 * r + bg * a)/255
...
void Fl_PS_Printer::draw(Fl_Bitmap * bitmap,int XP, int YP, int WP, int HP, int cx, int cy)
  ...
  if (WP> bitmap->w() - cx){// to assure that it does not go out of bounds
     w = bitmap->w() - cx
     xx = (bitmap->w()+7)/8 - cx/8; //length of mask in byte
  }else
    w =WP
    xx = (w+7)/8 - cx/8
  ...
  di += cy*LD + cx/8
  int si = cx % 8; // small shift to be clipped, it is simpler than shifting whole mas
  ...
  for (j=0; j<HP; j++)
    for (i=0; i<xx; i++)
      if (!(i%80)) fprintf(output, "\n"); // not have lines longer than 255 char
