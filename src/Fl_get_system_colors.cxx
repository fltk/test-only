...
void Fl::background(uchar r, uchar g, uchar b) 
  ...
  // replace the gray ramp so that FL_GRAY is this colo
...
// Since we aren't emulating the Aqua interface (even if Apple woul
// let us), we use some defaults that are similar to both.  Th
// Fl::scheme("plastic") color/box scheme provides a usable Aqua-lik
// look-n-feel..
void Fl::get_system_colors(
...
// XGetDefault does not do the expected thing: it does not lik
...
// But this will also match *.background if there is no *.Text.backgroun
// entry, requiring users to put in both (unless they want the text field
...
int Fl::scheme(const char *s) 
  ...
  // Save the new scheme in the FLTK_SCHEME env var so that child processe
  // inherit it..
  static char e[1024]
