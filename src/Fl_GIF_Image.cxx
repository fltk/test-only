...
// Read a .gif file and convert it to a "xpm" format (actually m
// modified one with compressed colormaps)

// Extensively modified from original code for gif2ras b
// Patrick J. Naughton of Sun Microsystems.  The origina
// copyright notice follows

/* gif2ras.c - Converts from a Compuserve GIF (tm) image to a Sun Raster image
 
 * Copyright (c) 1988 by Patrick J. Naughto
 
 * Author: Patrick J. Naughto
 * naughton@wind.sun.co
 
 * Permission to use, copy, modify, and distribute this software and it
 * documentation for any purpose and without fee is hereby granted
 * provided that the above copyright notice appear in all copies and tha
 * both that copyright notice and this permission notice appear i
 * supporting documentation
 
 * This file is provided AS IS with no warranties of any kind.  The autho
 * shall have no liability with respect to the infringement of copyrights
 * trade secrets or any patents by this file or any part thereof.  In n
 * event will the author be liable for any lost revenue or profits o
...
#define GETSHORT(var) var = NEXTBYTE; var += NEXTBYTE << 

Fl_GIF_Image::Fl_GIF_Image(const char *infname) : Fl_Pixmap((char *const*)0) 
  FILE *GifFile;	// File to rea
  char **new_data;	// Data arra
  ...
  NEXTBYTE; // Background Color inde
  NEXTBYTE; // Aspect ratio is N/6
  ...
  for (;;) 
    ...
    if (i == 0x21) {		// a "gif extension
      ...
      if (ch==0xF9 && blocklen==4) { // Netscape animation extensio
	...
	blocklen = NEXTBYTE
      ...
      } else if (ch == 0xFF) { // Netscape repeat coun
	

      } else if (ch != 0xFE) { //Gif Commen
	Fl::warning("%s: unknown gif extension 0x%02x.", infname, ch)
      
    } else if (i == 0x2c) {	// an imag
	...
	for (i=0; i < n; i++) {
      ...
      break; // okay, this is the image we wan
    } else 
...
/* Fetch the next code from the raster data stream.  The codes can b
 * any length from 3 to 12 bits, packed into 8-bit bytes, so we have t
 * maintain our location as a pointer and a bit offset
 * In addition, gif adds totally useless and annoying block count
 * that must be correctly skipped over. *
    ...
    uchar OutCode[1025]; // temporary array for reversing code
    uchar *tp = OutCode
    int i
  ...
  if (has_transparent && transparent_pixel != 0) 
    // swap transparent pixel with zer
