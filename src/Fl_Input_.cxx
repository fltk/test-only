...
// This is the base class for Fl_Input.  You can use it directl
// if you are one of those people who like to define their ow
// set of editing keys.  It may also be useful for adding scrollbar
// to the input field
...
// as necessary.  Truncate if necessary so the resulting string an
...
// After filling in such a buffer, find the width to 
double Fl_Input_::expandpos
  const char* p,	// real strin
  const char* e,	// pointer into real strin
  const char* buf,	// conversion of real string by expand(
  int* returnn		// return offset into buf her
) const 
  int n = 0
...
// Right now minimal update just keeps unchanged characters fro
// being erased, so they don't blink

void Fl_Input_::minimal_update(int p) 
  if (damage() & FL_DAMAGE_ALL) return; // don't waste time if it won't be don
...
void Fl_Input_::drawtext(int X, int Y, int W, int H) 
  ...
  if (Fl::focus()!=this && !size()) 
    if (do_mu) { // we have to erase it if cursor was ther
  ...
  for (; ypos < H;) 
    ...
    if (ypos <= -height) goto CONTINUE; // clipped off to

    if (do_mu) {	// for minimal update
      const char* pp = value()+mu_p; // pointer to where minimal update start
      if (e < pp) goto CONTINUE2; // this line is before the change
      if (readonly()) erase_cursor_only = 0; // this isn't the most efficient wa
      if (erase_cursor_only && p > pp) goto CONTINUE2; // this line is afte
	...
	if (erase_cursor_only) r = xx+2
      ...
      draw_box(box(), X-Fl::box_dx(box()), Y-Fl::box_dy(box())
               W+Fl::box_dw(box()), H+Fl::box_dh(box()), color())
      // it now draws entire line over i
      // this should not draw letters to left of erased area, bu
      // that is nyi
  ...
  if (input_type()==FL_MULTILINE_INPUT && do_mu && ypos<
...
void Fl_Input_::handle_mouse(int X, int Y, int /*W*/, int /*H*/, int drag) 
  ...
  if (Fl::event_clicks()) 
    ...
    // if the multiple click does not increase the selection, rever
...
int Fl_Input_::position(int p, int m) 
  ...
  if (p != m) 
    ...
    // new position is a curso
    if (position_ == mark_) 
      // old position was just a curso
      if (Fl::focus() == this && !(damage()&FL_DAMAGE_EXPOSE)) 
	minimal_update(position_); erase_cursor_only = 1
      
    } else { // old position was a selectio
...
static int yankcut;	// length of valid contents of buffer, even if undocut=
...
int Fl_Input_::replace(int b, int e, const char* text, int ilen) 
  ...
  if (e<=b && !ilen) return 0; // don't clobber undo for a null operatio
  ...
  // Insertions into the word at the end of the line will cause it t
  // wrap to the next line, so we must indicate that the changes may star
  // right after the whitespace before the current word.  This wil
  // result in sub-optimal update when such wrapping does not happe
...
int Fl_Input_::copy_cuts() 
  // put the yank buffer into the X clipboar
...
int Fl_Input_::handletext(int event, int X, int Y, int W, int H) 
  ...
  case FL_PASTE: 
    ...
    if (!t || e <= t) return 1; // Int/float stuff will crash without this tes
...
: Fl_Widget(X, Y, W, H, l) 
  ...
  cursor_color_ = FL_FOREGROUND_COLOR; // was FL_BLU
