...
#define DISPLAY_SEARCH_BOTH_WAYS_AT_ONC
...
// This is the base class for browsers.  To be useful it must b
// subclassed and several virtual functions defined.  Th
// Forms-compatable browser and the file chooser's browser ar
// subclassed off of this

// Yes, I know this should be a template..

// This has been designed so that the subclass has complete contro
// over the storage of the data, although because next() and prev(
// functions are used to index, it works best as a linked list or as 
// large block of characters in which the line breaks must be searche
// for

// A great deal of work has been done so that the "height" of a dat
// object does not need to be determined until it is drawn.  This wa
// done for the file chooser, because the height requires doing stat(
// to see if the file is a directory, which can be annoyingly slo
// over the network

/* redraw bits
   1 = redraw children (the scrollbar
   2 = redraw one or two item
   4 = redraw all item
*
...
// The scrollbars may be moved again by draw(), since each one's siz
// depends on whether the other is visible or not.  This skips ove
...
void Fl_Browser_::update_top() 
  ...
  if (position_ != real_position_) 
	...
	hh = item_quick_height(l)
      ...
      for (;;) 
	hh = item_height(l)
	if ((ly+hh) > yy) break; // it is big enough to se
	// go up to top of previous item
	void* l1 = item_prev(l)
	if (!l1) {ly = yy = 0; break;} // hit the to
...
// Change position(), top() will update when update_top() is calle
...
void Fl_Browser_::display(void* p) 
  ...
  if (l == p) {position(real_position_+Y); return;} // scroll up a bi
...
#ifdef DISPLAY_SEARCH_BOTH_WAYS_AT_ONC
  // search for item.  We search both up and down the list at the same time
  // this evens up the execution time for the two cases - the old way wa
  // much slower for going up than for going down
  while (l || lp) 
    if (l) 
      h1 = item_quick_height(l)
      if (l == p) 
	if (Y <= H) { // it is visible or right at botto
	  Y = Y+h1-H; // find where bottom edge i
	  if (Y > 0) position(real_position_+Y); // scroll down a bi
	} else 
	  position(real_position_+Y-(H-h1)/2); // center i
...
void Fl_Browser_::draw() 
  int drawsquare = 0
  if (damage() & FL_DAMAGE_ALL) { // redraw the box if full redra
    Fl_Boxtype b = box() ? box() : FL_DOWN_BOX
...
J1
  ...
  // Check the vertical scrollbar again, just in case it needs to be draw
  // because the horizontal one is drawn.  There should be a cleaner wa
  ...
  fl_clip(X, Y, W, H)
  // for each line, draw it if full redraw or scrolled.  Erase backgroun
  ...
  if (!dont_repeat) 
    dont_repeat = 1
    // see if changes to full_height caused by calls to slow_heigh
...
// Tell it that this item is going away, and that this must remov
// all pointers to it
...
int Fl_Browser_::handle(int event) 
  ...
  case FL_DRAG
    ...
    if (type() == FL_NORMAL_BROWSER || !top_
      ...
      void* l = find_item(my)
      void* t; void* b; // this will be the range to chang
      if (my > py) { // go dow
