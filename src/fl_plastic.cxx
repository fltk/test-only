...
/
// These box types provide a cross between Aqua and KDE buttons; kindo
...
// These box types are in seperate files so they are not linke
// in if not used
...
/
// Uncomment the following line to restore the old plastic box typ
// appearance
/

//#define USE_OLD_PLASTIC_BO
#define USE_OLD_PLASTIC_COLO

extern uchar *fl_gray_ramp()

inline Fl_Color shade_color(uchar gc, Fl_Color bc) 
#ifdef USE_OLD_PLASTIC_COLO
...
static void shade_frame(int x, int y, int w, int h, const char *c, Fl_Color bc) 
  ...
  for (x += b, y += b, w -= 2 * b, h -= 2 * b; b > 1; b --
    ...
    // Draw lines around the perimeter of the button, 4 colors pe
    // circuit
