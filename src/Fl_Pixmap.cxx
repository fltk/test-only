...
// Draws X pixmap data, keeping it stashed in a server pixmap so i
// redraws fast

// See fl_draw_pixmap.cxx for code used to get the actual data into pixmap
// Implemented without using the xpm library (which I can't use becaus
...
void Fl_Pixmap::copy_data() 
  ...
  char		**new_data,	// New data arra
		**new_row;	// Current row in imag
  int		i,		// Looping va
		ncolors,	// Number of colors in imag
		chars_per_pixel,// Characters per colo
...
Fl_Image *Fl_Pixmap::copy(int W, int H) 
  ...
  char		**new_data,	// New array for image dat
		**new_row,	// Pointer to row in image dat
		*new_ptr,	// Pointer into new arra
		new_info[255];	// New information lin
  const char	*old_ptr;	// Pointer into old arra
  int		i,		// Looping va
		c,		// Channel numbe
		sy,		// Source coordinat
  ...
  int		ncolors,	// Number of colors in imag
		chars_per_pixel,// Characters per colo
		chars_per_line;	// Characters per line
...
void Fl_Pixmap::color_average(Fl_Color c, float i) 
  ...
  char		line[255];	// New colormap lin
  int		color,		// Looping va
		ncolors,	// Number of colors in imag
		chars_per_pixel;// Characters per colo
...
void Fl_Pixmap::set_data(const char * const * p) 
  int	height,		// Number of lines in imag
	ncolors;	// Number of colors in imag
