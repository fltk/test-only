...
#  define NoValue	0x000
#  define XValue  	0x000
#  define YValue	0x000
#  define WidthValue  	0x000
#  define HeightValue  	0x000
#  define AllValues 	0x000
#  define XNegative 	0x001
#  define YNegative 	0x002
#endi
...
int Fl::arg(int argc, char **argv, int &i) 
  ...
  // a word that does not start with '-', or a word after a '--', o
  // the word '-' by itself all start the "non-switch arguments" t
  // a program.  Return 0 to indicate that we don't understand th
  // word, but set a flag (return_i) so that args() will return a
  // that point
  if (s[0] != '-' || s[1] == '-' || !s[1]) {return_i = 1; return 0;
  s++; // point after the das
...
#ifdef __APPLE_
  // The Finder application in MacOS X passes the "-psn_N_NNNNN" optio
  // to all apps..
  ...
  if (i >= argc-1 || !v
    return 0;	// all the rest need an argument, so if missing it is an erro
...
// Returns zero on error.  'i' will either point at first word tha
// does not start with '-', at the error word, or after a '--', or a
// argc.  If your program does not take any word arguments you ca
...
// show a main window, use any parsed argument
...
/
Permission to use, copy, modify, distribute, and sell this software and it
...
copyright notice and this permission notice appear in supportin
documentation, and that the name of M.I.T. not be used in advertising o
publicity pertaining to distribution of the software without specific
written prior permission.  M.I.T. makes no representations about th
suitability of this software for any purpose.  It is provided "as is
without express or implied warranty
*

/
 *    XParseGeometry parses strings of the for
 *   "=<width>x<height>{+-}<xoffset>{+-}<yoffset>", wher
 *   width, height, xoffset, and yoffset are unsigned integers
 *   Example:  "=80x24+300-49
 *   The equal sign is optional
 *   It returns a bitmask that indicates which of the four value
 *   were actually found in the string.  For each value found
 *   the corresponding argument is updated;  for each valu
...
int XParseGeometry(const char* string, int* x, int* y
  ...
  /* If strind isn't at the end of the string the it's an invali
