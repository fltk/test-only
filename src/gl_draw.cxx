...
// See also Fl_Gl_Window and gl_start.cx
...
void  gl_font(int fontid, int size) 
  ...
  if (!fl_fontsize->listbase) 
    ...
    // undefined characters automatically receive an empty GL list in aglUseFon
    fl_fontsize->listbase = glGenLists(256)
...
# if HAVE_G
  ...
  // clear variables used mostly in fl_fon
  fl_font_ = 0
  fl_size_ = 0
  ...
  for (int j = 0 ; j < FL_FREE_FONT ; ++j
    ...
    while (f != 0) 
      if(f->listbase) 
        ...
        // It would be nice if this next line was in a descturctor somewher
...
void gl_draw
  const char* str, 	// the (multi-line) strin
  int x, int y, int w, int h, 	// bounding bo
  Fl_Align align) 
...
void gl_color(Fl_Color i) 
  ...
  if (fl_overlay && fl_overlay_depth) 
    if (fl_overlay_depth < 8) 
      // only black & white produce the expected colors.  This coul
      // be improved by fixing the colormap set in Fl_Gl_Overlay.cx
