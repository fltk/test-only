...
// CodeWarrior (__MWERKS__) gets its include paths confused, so w
// temporarily disable this..
#if defined(__APPLE__) && !defined(__MWERKS__
#  include <sys/param.h
#  include <sys/ucred.h
#  include <sys/mount.h
#endif // __APPLE__ && !__MWERKS_
...
struct FL_BLINE			// data is in a linked list of thes

  FL_BLINE	*prev;		// Previous item in lis
  FL_BLINE	*next;		// Next item in lis
  void		*data;		// Pointer to data (function
  short		length;		// sizeof(txt)-1, may be longer than strin
  char		flags;		// selected, displaye
  char		txt[1];		// start of allocated arra
}
...
int					// O - Height in pixel
Fl_File_Browser::full_height() cons
...
int					// O - Height in pixel
Fl_File_Browser::item_height(void *p) const	// I - List item dat

  FL_BLINE	*line;			// Pointer to lin
  char		*t;			// Pointer into tex
  int		height;			// Width of lin
  int		textheight;		// Height of tex
...
int					// O - Width in pixel
Fl_File_Browser::item_width(void *p) const	// I - List item dat

  int		i;			// Looping va
  FL_BLINE	*line;			// Pointer to lin
  char		*t,			// Pointer into tex
		*ptr,			// Pointer into fragmen
		fragment[10240];	// Fragment of tex
  int		width,			// Width of lin
		tempwidth;		// Width of fragmen
  int		column;			// Current colum
  const int	*columns;		// Column
...
voi
Fl_File_Browser::item_draw(void *p,	// I - List item dat
                 	   int  X,	// I - Upper-lefthand X coordinat
		 	   int  Y,	// I - Upper-lefthand Y coordinat
		 	   int  W,	// I - Width of ite
			   int) const	// I - Height of ite

  int		i;			// Looping va
  FL_BLINE	*line;			// Pointer to lin
  Fl_Color	c;			// Text colo
  char		*t,			// Pointer into tex
		*ptr,			// Pointer into fragmen
		fragment[10240];	// Fragment of tex
  int		width,			// Width of lin
		height;			// Height of lin
  int		column;			// Current colum
  const int	*columns;		// Column
...
Fl_File_Browser::Fl_File_Browser(int        X,  // I - Upper-lefthand X coordinat
                        	 int        Y,  // I - Upper-lefthand Y coordinat
				 int        W,  // I - Width in pixel
				 int        H,  // I - Height in pixel
				 const char *l)	// I - Label tex
...
int						// O - Number of files loade
Fl_File_Browser::load(const char     *directory,// I - Directory to loa
                      Fl_File_Sort_F *sort)	// I - Sort function to us

  int		i;				// Looping va
  int		num_files;			// Number of files in director
  int		num_dirs;			// Number of directories in lis
  char		filename[4096];			// Current fil
  Fl_File_Icon	*icon;				// Icon to us
...
//  printf("Fl_File_Browser::load(\"%s\")\n", directory)
  ...
  if (directory_[0] == '\0'
    ...
    /
    // No directory specified; for UNIX list all mount points.  For DO
...
#  ifdef __CYGWIN_
    /
    // Cygwin provides an implementation of setmntent() to get the lis
    // of available drives..
    /
...
#  els
    ...
    /
    DWORD	drives;		// Drive available bit
...
#  endif // __CYGWIN_
    ...
    /
    ULONG	curdrive;	// Current driv
    ULONG	drives;		// Drive available bit
    ...
    int			numfs;	// Number of file system
    struct statfs	*fs;	// Buffer for file system inf
    ...
    /
    FILE	*mtab;		// /etc/mtab or /etc/mnttab fil
    ...
    mtab = fopen("/etc/mnttab", "r");	// Fairly standar
    if (mtab == NULL
      mtab = fopen("/etc/mtab", "r");	// More standar
    if (mtab == NULL
      mtab = fopen("/etc/fstab", "r");	// Otherwise fallback to full lis
    if (mtab == NULL
      mtab = fopen("/etc/vfstab", "r");	// Alternate full list fil
...
//        printf("Fl_File_Browser::load() - adding \"%s\" to list...\n", filename)
  ...
  els
    ...
    dirent	**files;	// Files in in director
	...
	if (fl_filename_isdir(filename)
	  ...
	  char name[1024]; // Temporary directory nam
...
voi
Fl_File_Browser::filter(const char *pattern)	// I - Pattern strin
