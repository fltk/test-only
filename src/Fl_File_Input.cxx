...
Fl_File_Input::update_buttons() 
  int		i;				// Looping va
  const char	*start,				// Start of path componen
		*end;				// End of path componen
...
int						// O - TRUE on succes
Fl_File_Input::value(const char *str,		// I - New string valu
                     int        len) {		// I - Length of valu
...
int						// O - TRUE if we handled even
Fl_File_Input::handle(int event) 		// I - Even
...
int						// O - TRUE if we handled even
Fl_File_Input::handle_button(int event)		// I - Even

  int		i,				// Looping va
		X;				// Current X positio
  char		*start,				// Start of path componen
		*end;				// End of path componen
  char		newvalue[1024];			// New valu
...
//  printf("handle_button(event = %d), button = %d\n", event, i)
  ...
  // Return immediately if the user is clicking on the last button o
