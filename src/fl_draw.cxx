...
char fl_draw_shortcut;	// set by fl_labeltypes.cx
...
expand(const char* from, char* buf, double maxw, int& n, double &width
  ...
  for (;; p++) 
	...
	if (word_end > buf && newwidth > maxw) { // break before this wor
	  o = word_end
	  p = word_start
	  break
    ...
    if (o > e) break; // don't overflow buffe
    ...
    if (c == '\t') 
      ...
      *o++ = '^'
      *o++ = c ^ 0x40
    } else if (c == 0xA0) { // non-breaking spac
      *o++ = ' '
