...
// "$Id$
/
// Carbon image reading routines for the Fast Light Tool Kit (FLTK)
/
...
fl_read_image(uchar *p,		// I - Pixel buffer or NULL to allocat
  ...
  Rect		src,		// Source rectangl
		dst;		// Destination rectangl
  Fl_Offscreen	osbuffer;	// Temporary off-screen buffer for cop
  GrafPtr	srcPort;	// Source por
  RGBColor	rgb;		// RGB colors for copy mask..
  PixMapHandle	pm;		// Pixmap handle for off-screen buffe
  uchar		*base,		// Base address of off-screen buffe
		*psrc,		// Pointer into off-screen buffe
		*pdst;		// Pointer into pixel buffe
  int           idx, idy;	// Current X & Y in imag
  int		d;		// Depth of imag
  ...
  // Set the correct port for the off-screen buffer and lock the buffe
