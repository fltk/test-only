...
//: MeasureText, FontMetrics, WidthTabHandle, GetSysFont, SysFontSiz
//: TextSize, TextFon
...
Fl_FontSize::Fl_FontSize(const char* name, int Size) 
  ...
  ascent = fOut->ascent;	//: the following three lines give only temporary aproimation
...
/
#if HAVE_G
 // ++ todo: remove OpenGL font alocation
...
void fl_font(Fl_FontSize *s) 
  ...
  TextFont(fl_fontsize->font);	//: select font into current QuickDraw G
  ...
  if (!fl_fontsize->knowMetrics) {	//: get the true metrics for the currnet GC (fails on multiple monitors with different dpi's!
    ...
    short *f = (short*)*mr.wTabHandle; //: get the char size tabl
...
void Fl_Carbon_Display::font(int fnum, int size) 
  fl_font_ = fnum
  fl_size_ = size
...
// todo : fl_width returns wrong results for OS 
double Fl_Carbon_Display::width(unsigned c) 
  return (double)TextWidth( &c, 0, 1 )


// MRS: The default character set is MacRoman, which is different fro
...
static uchar macroman_lut[256] = 
  ...
  240, 150, 152, 151, 153, 155, 154, 214, 191, 157, 156, 158, 159, 253, 254, 21
}

void Fl_Carbon_Display::draw(const char* str, int n, int x, int y) 
  int	i;				// Looping va
  uchar	buf[1024],			// Temporary buffe
	*bufptr;			// Pointer into buffe
