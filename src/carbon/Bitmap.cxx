...
Fl_Bitmask fl_create_bitmask(int w, int h, const uchar *array) 
  ...
  GetPort(&savePort); // remember the current por
  ...
  if ( pm )
    ...
    if ( *pm )
      ...
      if ( base )
        ...
        PixMapPtr pmp = *pm
        // verify the parameters for direct memory writ
...
//COPYDEEPMAS
#if 
// Create an 8-bit mask used for alpha blendin
Fl_Bitmask fl_create_alphamask(int w, int h, int d, int ld, const uchar *array) 
  Rect srcRect
  srcRect.left = 0; srcRect.right = w
  srcRect.top = 0; srcRect.bottom = h
...
void Fl_Carbon_Display::draw(Fl_Bitmap * img, int X, int Y, int W, int H, int cx, int cy) 
  ...
  CopyBits(GetPortBitMapForCopyBits((Fl_Offscreen)(cache->id)),	// srcBit
	   GetPortBitMapForCopyBits(dstPort),	// dstBit
	   &src,		 		// src bound
	   &dst, 				// dst bound
	   srcOr, 				// mod
	   0L);					// mask regio
