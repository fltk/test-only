...
// we don't need the following definition because we deliver onl
// true mouse moves.  On very slow systems however, this flag ma
// still be useful
...
// #define DEBUG_SELECT		// UNCOMMENT FOR SELECT()/THREAD DEBUGGIN
#ifdef DEBUG_SELEC
#include <stdio.h>		// testin
#define DEBUGMSG(msg)		if ( msg ) fprintf(stderr, msg)
#define DEBUGPERRORMSG(msg)	if ( msg ) perror(msg
#define DEBUGTEXT(txt)		tx
#els
#define DEBUGMSG(msg
#define DEBUGPERRORMSG(msg
#define DEBUGTEXT(txt)		NUL
#endif /*DEBUG_SELECT*
...
// forward definition of functions in this fil
...
WindowRef fl_capture = 0;            // we need this to compensate for a missing(?) mouse captur
ulong fl_event_time;                 // the last timestamp from an x even
char fl_key_vector[32];              // used by Fl::get_key(
bool fl_show_iconic;                 // true if called from iconize() - shows the next created window in collapsed stat
int fl_disable_transient_for;        // secret method of removing TRANSIENT_FO
const Fl_Window* fl_modal_for;       // parent of modal() windo
Fl_Region fl_window_region = 0
Window fl_window
Fl_Window *Fl_Window::current_
EventRef fl_os_event;		// last (mouse) even

// forward declarations of variables in this fil
static int got_events = 0
static Fl_Window* resize_from_system
static CursPtr default_cursor_ptr
static Cursor default_cursor
static WindowRef fl_os_capture = 0; // the dispatch handler will redirect mose move and drag events to these window
...
//     When the user app configures one or more file descriptors to monito
...
//     RunApplicationLoop(), so that it triggers the data  ready  callback
//     in the parent thread.                               -erco 04/04/0
//    
#define POLLIN  
#define POLLOUT 
#define POLLERR 
...
class DataRead
    ...
    FD *fds
    pthread_t tid;		// select()'s thread i

    // Data that needs to be locked (all start with '_'
    pthread_mutex_t _datalock;	// data loc
    fd_set _fdsets[3];		// r/w/x sets user wants to monito
    int _maxfd;			// max fd count to monito
    int _cancelpipe[2];		// pipe used to help cancel threa
    void *_userdata;		// thread's userdat
...
public
    ...
    //    with /*LOCK*/ to make painfully obvious esp. in debuggers. -erc
    /
...
// CHECK IF USER DATA READY, RETURNS r/w/x INDICATING WHICH IF AN
int DataReady::CheckData(fd_set& r, fd_set& w, fd_set& x

  int ret
  DataLock()
  /*LOCK*/  timeval t = { 0, 1 };		// quick chec
...
void* DataReady::DataReadyThread(void *o
  ...
  while ( 1 )					// loop until thread cancel or erro
    ...
    /*LOCK*/  FD_SET(cancelpipe, &r);		// add cancelpipe to fd's to watc
    /*LOCK*/  FD_SET(cancelpipe, &x)
    self->DataUnlock()
    // timeval t = { 1000, 0 };	// 1000 seconds
    timeval t = { 2, 0 };	// HACK: 2 secs prevents 'hanging' proble
    int ret = ::select(maxfd+1, &r, &w, &x, &t)
    pthread_testcancel();	// OSX 10.0.4 and older: needed for parent to cance
    switch ( ret 
	...
	if (FD_ISSET(cancelpipe, &r) || FD_ISSET(cancelpipe, &x)) 	// cancel
        ...
        return(NULL);		// done with threa
      
    
  


// START 'DATA READY' THREAD RUNNING, CREATE INTER-THREAD PIP
void DataReady::StartThread(void *new_userdata

  CancelThread(DEBUGTEXT("STARTING NEW THREAD\n"))
  DataLock()
  /*LOCK*/  pipe(_cancelpipe);	// pipe for sending cancel msg to threa
...
// CANCEL 'DATA READY' THREAD, CLOSE PIP
void DataReady::CancelThread(const char *reason
  ...
  if ( tid 
    ...
    DEBUGMSG(reason)
    if ( pthread_cancel(tid) == 0 )		// cancel firs
    
      DataLock()
      /*LOCK*/  write(_cancelpipe[1], "x", 1);	// wake thread from selec
      DataUnlock()
      pthread_join(tid, NULL);			// wait for thread to finis
...
/*
 * handle Apple Menu items (can be created using the Fl_Sys_Menu_Ba
 * returns eventNotHandledErr if the menu item could not be handle
 *
OSStatus HandleMenu( HICommand *cmd 

  OSStatus ret = eventNotHandledErr
  // attributes, commandIDm menu.menuRef, menu.menuItemInde
  UInt32 ref
  ...
  if ( rrc==noErr && ref 
    ...
    if ( m->flags & FL_MENU_TOGGLE ) // update the menu toggle symbo
      SetItemMark( cmd->menu.menuRef, cmd->menu.menuItemIndex, (m->flags & FL_MENU_VALUE ) ? 0x12 : 0 )
    if ( m->flags & FL_MENU_RADIO ) // update all radio buttons in this men
    ...
    ret = noErr; // done handling this even
...
/*
 * We can make every event pass through this functio
 * - mouse events need to be manipulated to use a mouse focus windo
 * - keyboard, mouse and some window  events need to quit the Apple Event Loo
 *   so FLTK can continue its own managemen
 *
static pascal OSStatus carbonDispatchHandler( EventHandlerCallRef nextHandler, EventRef event, void *userData 
  ...
  case kEventClassFLTK
	...
	dataready.CancelThread(DEBUGTEXT("DATA READY EVENT\n"))
        ...
        // CHILD THREAD TELLS US DATA READ
	//     Check to see what's ready, and invoke user's cb'
	/
	fd_set r,w,x
  ...
  if ( ret == eventNotHandledErr 
    ret = CallNextEventHandler( nextHandler, event ); // let the OS handle the activation, but continue to get a click-through effec
...
/*
 * this callback simply quits the main event loop handler, so FLTK can do its magi
 *
 ...
 * It reads events from the event queue using the given maximum tim
...
static double do_queued_events( double time = 0.0 )
  ...
  // initialize events and a region that enables mouse move event
  if (!been_here) 
    rgn = NewRgn()
    Point mp
    GetMouse(&mp)
  ...
  RunApplicationEventLoop(); // will return after the previously set tim
  if ( dataready.IsThreadRunning() 
 ...
 * 'time' secods for an event. This version returns 1 if event
 * other than the timeout timer were processed
 
 * \todo there is no socket handling in this code whatsoeve
 *
...
/*
 * event handler for Apple-Q key combinatio
 * this is also called from the Carbon Window handler after all windows were close
 *
static OSErr QuitAppleEventHandler( const AppleEvent *appleEvt, AppleEvent* reply, UInt32 refcon 
  ...
  while ( Fl_X::first ) 
    ...
    if ( Fl_X::first == x ) 
      fl_unlock_function()
      return noErr; // FLTK has not close all windows, so we return to the main program no
...
/*
 * Carbon Window handle
 * This needs to be linked into all new window event handler
 *
static pascal OSStatus carbonWindowHandler( EventHandlerCallRef nextHandler, EventRef event, void *userData 
  ...
  case kEventWindowClose
    Fl::handle( FL_CLOSE, window ); // this might or might not close the windo
    // if there are no more windows, send a high-level quit even
    if (!Fl_X::first) QuitAppleEventHandler( 0, 0, 0 )
    ret = noErr; // returning noErr tells Carbon to stop following up on this even
    break
...
/*
 * convert the current mouse chord into the FLTK modifier stat
 *
static void chord_to_e_state( UInt32 chord 
  ...
  static ulong state[] =
  {
    ...
    FL_BUTTON2|FL_BUTTON1|FL_BUTTON
  }
...
static pascal OSStatus carbonMouseHandler( EventHandlerCallRef nextHandler, EventRef event, void *userData 
  ...
  case kEventMouseDown
    ...
    if ( part != inContent ) 
      fl_unlock_function()
      return CallNextEventHandler( nextHandler, event ); // let the OS handle this for u
    
    if ( !IsWindowActive( xid ) 
      CallNextEventHandler( nextHandler, event ); // let the OS handle the activation, but continue to get a click-through effec
    // normal handling of mouse-down follow
    fl_os_capture = xid
    sendEvent = FL_PUSH
  ...
  case kEventMouseDragged
    if ( !sendEvent ) 
      sendEvent = FL_MOVE; // Fl::handle will convert into FL_DRA
    ...
    GrafPtr oldPort
    GetPort( &oldPort )
    SetPort( GetWindowPort(xid) ); // \todo replace this! There must be some GlobalToLocal call that has a port as an argumen
    SetOrigin(0, 0)
    Fl::e_x_root = pos.h
    Fl::e_y_root = pos.v
    GlobalToLocal( &pos )
...
/*
 * convert the current mouse chord into the FLTK keysy
 *
...
/*
 * convert the keyboard return code into the symbol on the keycap
 *
...
pascal OSStatus carbonKeyboardHandler(
  ...
  // get the modifiers for any of the event
  GetEventParameter( event, kEventParamKeyModifiers, typeUInt32,
                     NULL, sizeof(UInt32), NULL, &mods )
  if ( prevMods == 0xffffffff ) prevMods = mods
 
  // get the key code only for key event
  UInt32 keyCode = 0
  unsigned char key = 0
  ...
  /* output a human readbale event identifier for debuggin
  const char *ev = ""
  switch (kind) 
  ...
  *
  ...
  case kEventRawKeyUp
    ...
    // but event_text should generate the international symbo
    if ( isalpha(key) 
      sym = tolower(key)
    else if ( Fl::e_state&FL_CTRL && key<32 
      sym = key+96
    else if ( Fl::e_state&FL_ALT ) // find the keycap of this ke
      sym = keycode_to_sym( keyCode & 0x7f, 0, macKeyLookUp[ keyCode & 0x7f ] )
    els
      sym = macKeyLookUp[ keyCode & 0x7f ]
    Fl::e_keysym = sym
    if ( keyCode==0x4c ) key=0x0d
    // Matt: the Mac has no concept of a NumLock key, or at least not visibl
    // Matt: to Carbon. The kEventKeyModifierNumLockMask is only set whe
    // Matt: a numeric keypad key is pressed and does not correspond wit
  ...
  if (sendEvent && Fl::handle(sendEvent,window)) 
    fl_unlock_function(); 
    return noErr; // return noErr if FLTK handled the even
  } else 
    fl_unlock_function()
    //return CallNextEventHandler( nextHandler, event );
    // Matt: I had better results (no duplicate events) always returnin
...
/*
 * Event handler for Apple-O key combination and also for file open
 * via the finder..
 *
...
/*
 * initialize the Mac toolboxes and set the default menuba
 *
...
/*
 * smallest x ccordinate in screen spac
 *
int Fl::x() 
  BitMap r
...
/*
 * smallest y ccordinate in screen spac
 *
int Fl::y() 
  BitMap r
  GetQDGlobalsScreenBits(&r)
...
/*
 * get the current mouse pointer world coordinate
 *
...
/*
 * Initialize the given port for redraw and call the windw's flush() to actually draw the conten
 */
void Fl_X::flush(
...
/*
 * Handle all clipping and redraw for the given por
 * There are two different callers for this event
 * 1: the OS can request a redraw and provides all clipping itsel
 * 2: Fl::flush() wants all redraws no
 */   
void handleUpdateEvent( WindowPtr xid )
  ...
  if ( i->region ) { // no region, so the sytem will take the update region from the O
    DisposeRgn( i->region )
    i->region = 0
...
int Fl_X::fake_X_wm(const Fl_Window* w,int &X,int &Y, int &bt,int &bx, int &by) 
  ...
  //The coordinates of the whole window, including non-client are
  xoff = bx
  yoff = by + bt
  dx = 2*bx
  dy = 2*by + bt
  ...
  //Proceed to positioning the window fully inside the screen, if possibl
 
    // let's get a little elaborate here. Mac OS X puts a lot of stuff on the des
  // that we want to avoid when positioning our window, namely the Dock and th
  // top menu bar (and even more stuff in 10.4 Tiger). So we will go through th
  // list of all available screens and find the one that this window is mos
  // likely to go to, and then reposition it to fit withing the 'good' area
  Rect r
  // find the screen, that the center of this window will fall int
  int R = X+W, B = Y+H; // right and botto
  int cx = (X+R)/2, cy = (Y+B)/2; // center of window
  GDHandle gd = 0L
  for (gd = GetDeviceList(); gd; gd = GetNextDevice(gd)) 
  GDPtr gp = *gd
  if (    cx >= gp->gdRect.left && cx <= gp->gdRect.righ
       && cy >= gp->gdRect.top  && cy <= gp->gdRect.bottom
    break
  
  // if the center doesn't fall on a screen, try the top lef
  if (!gd) 
    for (gd = GetDeviceList(); gd; gd = GetNextDevice(gd)) 
      GDPtr gp = *gd
      if (    X >= gp->gdRect.left && X <= gp->gdRect.righ
           && Y >= gp->gdRect.top  && Y <= gp->gdRect.bottom
        break
    
  
  // if that doesn't fall on a screen, try the top righ
  if (!gd) 
    for (gd = GetDeviceList(); gd; gd = GetNextDevice(gd)) 
      GDPtr gp = *gd
      if (    R >= gp->gdRect.left && R <= gp->gdRect.righ
           && Y >= gp->gdRect.top  && Y <= gp->gdRect.bottom
        break
    
  
  // if that doesn't fall on a screen, try the bottom lef
  if (!gd) 
    for (gd = GetDeviceList(); gd; gd = GetNextDevice(gd)) 
      GDPtr gp = *gd
      if (    X >= gp->gdRect.left && X <= gp->gdRect.righ
  ...
  // last resort, try the bottom righ
  if (!gd) 
    for (gd = GetDeviceList(); gd; gd = GetNextDevice(gd)) 
      GDPtr gp = *gd
      if (    R >= gp->gdRect.left && R <= gp->gdRect.righ
  ...
  // if we still have not found a screen, we will use the mai
...
static pascal OSErr dndTrackingHandler( DragTrackingMessage msg, WindowPtr w, void *userData, DragReference dragRef 
  ...
  case kDragTrackingInWindow
    GetDragMouse( dragRef, &mp, 0 )
    if ( mp.h==px && mp.v==py 
      break;	//+ return previous condition for dnd hilitin
    Fl::e_x_root = px = mp.h
    Fl::e_y_root = py = mp.v
    Fl::e_x = px - target->x()
    Fl::e_y = py - target->y()
...
static pascal OSErr dndReceiveHandler( WindowPtr w, void *userData, DragReference dragRef 
  ...
  for ( i = 1; i <= nItem; i++ 
    ...
    if ( ret == noErr 
      ...
      size += 1024; //++ ouch! We should create the full pathname and figure out its lengt
  ...
  dst[-1] = 0
//  if ( Fl::e_text[Fl::e_length-1]==0 ) Fl::e_length--; // modify, if trailing 0 is part of strin
...
/*
 * go ahead, create that (sub)windo
 * \todo we should make menu windows slightly transparent for the new Mac loo
 *
void Fl_X::make(Fl_Window* w

  static int xyPos = 50
  if ( w->parent() ) // create a subwindo
    ...
    if (xo) 
      ...
      Fl::e_number = old_event
      w->redraw(); // force draw to happe
    
    fl_show_iconic = 0
  
  else // create a desktop windo
    ...
    if (!(w->flags() & Fl_Window::FL_FORCE_POSITION)) 
      // use the Carbon functions below for default window positionin
      w->x(xyPos+Fl::x())
      w->y(xyPos+Fl::y())
      xyPos += 25
    ...
    Fl_X* x = new Fl_X
    x->other_xid = 0; // room for doublebuffering image map. On OS X this is only used by overlay window
    x->region = 0
    x->subRegion = 0
    x->cursor = fl_default_cursor
    x->xidChildren = 0
    ...
    winattr &= GetAvailableWindowAttributes( winclass );	// make sure that the window will ope
    CreateNewWindow( winclass, winattr, &wRect, &(x->xid) )
    SetWTitle(x->xid, pTitle)
    MoveWindow(x->xid, wRect.left, wRect.top, 1);	// avoid Carbon Bug on old O
    if (w->non_modal() && !w->modal()) 
      // Major kludge: this is to have the regular look, but stay above the document window
    ...
    if ( ! Fl_X::first->next ) // if this is the first window, we need to bring the application to the fron
    {
    ...
    if (fl_show_iconic) {
      fl_show_iconic = 0
      CollapseWindow( x->xid, true ); // \todo Mac ; unteste
...
/*
 * this is a leftover from X Window
 *
...
void Fl_Window::make_current()
  ...
  SetPort( GetWindowPort(i->xid) ); // \todo check for the handling of doublebuffered window
  ...
  // \todo for performance reasons: we don't have to create this unless the child windows move
  for ( Fl_X *cx = i->xidChildren; cx; cx = cx->xidNext 
...
/*
 * create a selectio
 * owner: widget that created the selectio
 * stuff: pointer to selected dat
 * size of selected dat
 *
void Fl::copy(const char *stuff, int len, int clipboard) 
  ...
  if (clipboard) 
    ...
    // Previous version changed \n to \r before sending the text, but I woul
    // prefer to leave the local buffer alone, so a copied buffer may b
