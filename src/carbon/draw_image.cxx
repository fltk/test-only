...
/*
 * draw an image based on the input parameter
 
 * buf:       image source dat
 * X, Y:      position (in buffer?!
 * W, H:      size of picture (in pixel?
 * delta:     distance from pixel to pixel in buf in byte
 * linedelta: distance from line to line in buf in byte
 * mono:      if set, pixel is one byte - if zero, pixel is 3 byt
 * cb:        callback to copy image data into (RGB?) buffe
 *   buf:       pointer to first byte in image sourc
 *   x, y:      position in buffe
 *   w:         width (in bytes?
 *   dst:       destination buffe
 * userdata:  
 *
static void innards(const uchar *buf, int X, int Y, int W, int H
  ...
  // theoretically, if the current GPort permits, we could writ
  // directly into it, avoiding the temporary GWorld. For now 
  ...
  if (err==noErr && gw) 
    ...
    if ( pm ) 
      ...
      if ( *pm ) 
        ...
        if ( base ) 
          PixMapPtr pmp = *pm
          // make absolutely sure that we can use a direct memory write t
          // create the pixmap
  ...
  // following the very save (and very slow) way to write the image into the give por
  if ( cb 
...
// \todo Mac : the above function does not support subregions ye
#ifdef later_we_do_thi
// \todo Mac : the following code is taken from fl_draw_image_win32 and needs to be modified for Mac Carbo
//  if (!linedelta) linedelta = W*delta
...
void Fl_Carbon_Display::draw_image(const uchar* buf, int x, int y, int w, int h, int d, int l)
  innards(buf,x,y,w,h,d,l,(d<3&&d>-3),0,0)

void Fl_Carbon_Display::draw_image(Fl_Draw_Image_Cb cb, void* data
		   int x, int y, int w, int h,int d) 
  innards(0,x,y,w,h,d,0,(d<3&&d>-3),cb,data)

void Fl_Carbon_Display::draw_image_mono(const uchar* buf, int x, int y, int w, int h, int d, int l)
  innards(buf,x,y,w,h,d,l,1,0,0)

void Fl_Carbon_Display::draw_image_mono(Fl_Draw_Image_Cb cb, void* data
		   int x, int y, int w, int h,int d) 
  innards(0,x,y,w,h,d,0,1,cb,data)


void Fl_Carbon_Display::rectf(int x, int y, int w, int h, uchar r, uchar g, uchar b) 
  fl_color(r,g,b)
  fl_rectf(x,y,w,h)
