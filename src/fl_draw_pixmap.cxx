...
// Current implementation is cheap and slow, and works best on a full-colo
// display.  Transparency is not handled, and colors are dithered t
// the color cube.  Color index is achieved by adding the i
...
uchar **fl_mask_bitmap; // if non-zero, create bitmap and store pointer her
...
int fl_draw_pixmap(const char*const* di, int x, int y, uchar bg_r, uchar bg_g, uchar bg_b) 
  ...
  if (ncolors < 0) {	// FLTK (non standard) compressed colorma
    ncolors = -ncolors
    const uchar *p = *data++
    // if first color is ' ' it is transparent (put it later to mak
...
#  endi
  ...
  } else {	// normal XPM colormap with name
...
#  endi
      ...
      if (!fl_parse_color((const char*)p, c[0], c[1], c[2])) 
        // assume "None" or "#transparent" for any error
