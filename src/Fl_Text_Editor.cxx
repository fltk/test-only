...
int Fl_Text_Editor::handle_key() 
  ...
  // This uses the right-hand ctrl key as a "compose prefix" and return
  // the changes that should be made to the text, as a number o
  ...
  state &= FL_SHIFT|FL_CTRL|FL_ALT|FL_META; // only care about these state
  Key_Func f
...
int Fl_Text_Editor::handle(int event) 
  ...
  switch (event) 
    case FL_FOCUS
      show_cursor(mCursorOn); // redraws the curso
